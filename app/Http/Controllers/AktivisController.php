...
use App\Models\AktivisKeterangan;
...
class AktivisController extends Controller
{
    ...
    public function indexTingkatArr($kegiatan_id, $tingkat)
    {
        ...
        $table_data = Aktivis::with(['pekerjaans' => function ($q) use ($tingkat) {
            ...
            $query->whereIn('tipe', [1, 3])
                ->whereIn('tingkat', $tingkat)
                ->where('status', 1);
        })->whereNotIn('id', $kegiatanPeserta)->advancedFilter([], request()->all());
    ...
    public function history()
    {
        ...
        $table_data = Revision::with('revisionable')->where('revisionable_type','App\Models\Aktivis')->where('created_at','>=',$time)->orderBy('created_at','desc')->take(5);
