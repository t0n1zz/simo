...
use App\Exports\MonitoringCu as ExportMonitoringCu;
use App\Exports\MonitoringKonsolidasiCU;
...
class MonitoringCuController extends Controller{
	protected $message = "MonitoringCu";

	public function index(Request $request, $status)
	{
		if ($status == 'selesai') {
			$table_data = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa', 'monitoring_rekom')
			...
			$table_data = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa')->withCount('monitoring_pencapaian', 'monitoring_rekom', 'monitoring_rekom_ok')->advancedFilter();
		}
	...
	public function indexKonsolidasi(Request $request, $tahun, $bulan)
	{
		...
		$table_data = Cu::with('hasMonitoringCu', 'hasMonitoringCu.monitoring_rekom')->whereNull('deleted_at')->get();

		$table_data->each(function ($cu) use ($date, $dateLY, $dateLM) {
			$cu->hasMonitoringCu->each(function ($monitoring) use ($date, $dateLM, $dateLY) {
		...
		$table_data->each(function ($cu) use (&$sum_tercapai, &$sum_tidak_tercapai, &$sum_rekom) {
			...
			$cu->jumlah_rekomendasi = $cu->hasMonitoringCu->sum('monitoring_rekom_count');
			$cu->tercapai = $cu->hasMonitoringCu->sum('monitoring_rekom_ok_count');
			...
			$cu->presentase_rekom_ok_tahun_lalu = $cu->rekom_des_tahun_lalu != 0 && $cu->hasMonitoringCu->sum('monitoring_rekom_ok_last_year_count') != 0 ? round(($cu->hasMonitoringCu->sum('monitoring_rekom_ok_last_year_count') / $cu->rekom_des_tahun_lalu) * 100, 2) : 0;
			$cu->presentase_rekom_ok_bulan_lalu = $cu->hasMonitoringCu->sum('monitoring_rekom_last_month_count') != 0 && $cu->hasMonitoringCu->sum('monitoring_rekom_ok_last_month_count') != 0 ?
				round(($cu->hasMonitoringCu->sum('monitoring_rekom_ok_last_month_count') / $cu->hasMonitoringCu->sum('monitoring_rekom_last_month_count')) * 100, 2) : 0;
	...
	public function indexCu(Request $request,$cu, $tp, $status)
	{
		if ($tp != 'semua') {
			if($status == 'semua'){
				$table_data = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa')->withCount('monitoring_pencapaian', 'monitoring_rekom', 'monitoring_rekom_ok')->where('id_cu', $cu)->where('id_tp', $tp)->advancedFilter();
			...
			if ($status == 'semua') {
				$table_data = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa', 'monitoring_rekom')->withCount('monitoring_pencapaian', 'monitoring_rekom', 'monitoring_rekom_ok')->where('id_cu', $cu)->advancedFilter();
			} elseif ($status == 'selesai') {
				$table_data = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa', 'monitoring_rekom')
	...
	public function summary($id_cu){
		$sum_tercapai= 0;
		$sum_rekom = 0;
		$data_sum = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa', 'monitoring_rekom')->withCount('monitoring_pencapaian', 'monitoring_rekom', 'monitoring_rekom_ok')->where('id_cu', $id_cu)->get();
	...
	{
		$table_data = MonitoringCu::where('id_cu',$id)->select('id','id_cu','name','tanggal')->orderBy('tanggal','desc')->get();
	...
	{
		$form = MonitoringCu::initialize();

		return response()
			->json([
					'form' => $form,
					'rules' => MonitoringCu::$rules,
					'option' => []
			]);
	}
	...
	public function store(Request $request)
	{
		\DB::beginTransaction(); 
		try{
			$this->validate($request,MonitoringCu::$rules);

			$name = $request->name;

			$kelas = MonitoringCu::create($request->except('rekomendasi'));
	...
	{
		$kelas = MonitoringCu::with('cu','tp','aktivis_pic.pekerjaan_aktif','aktivis_pemeriksa.pekerjaan_aktif','monitoring_rekom')->findOrFail($id);
	...
	public function update(Request $request, $id)
	{
		...
		try{
			...
			$kelas = MonitoringCu::findOrFail($id);
	...
	{
		$kelas = MonitoringRekomCu::findOrFail($id);
	...
	public function destroy($id)
	{
		...
		try{
			$kelas = MonitoringCu::findOrFail($id);
			$kelas2 = MonitoringRekomCu::where('id_monitoring',$id);
			$kelas3 = MonitoringPencapaianCu::where('id_monitoring',$id);
		...
		if($request->rekomendasi){
			...
			foreach($rekoms as $rekom){
				if(array_key_exists('id', $rekom)){
					$kelasRekom = MonitoringRekomCu::findOrFail($rekom['id']);
					$kelasRekom->update([
						'id_monitoring_cu' => $kelas->id,
	...
	public function history()
  {
    ...
    $table_data = Revision::with('revisionable')->where('revisionable_type','App\Models\MonitoringCu')->where('created_at','>=',$time)->orderBy('created_at','desc')->take(5);
	...
	public function downloadLaporan(Request $request)
	{
		...
		$sum_rekom = 0;
		$sum_pencapaian = 0;
		$data_sum = MonitoringCu::with('cu', 'tp', 'aktivis_pic', 'aktivis_pemeriksa', 'monitoring_rekom')->withCount('monitoring_pencapaian', 'monitoring_rekom', 'monitoring_rekom_ok')->where('id_cu', $request->id_cu)->get();
		...
		if ($request->id_tp != 'semua') {
			if ($request->semua == true) {
				$count_temuan = MonitoringCu::where('id_cu', $request->id_cu)->where('id_tp', $request->id_tp)->orderBy('aspek', 'desc')->count();
			} else {
				$count_temuan = MonitoringCu::where('id_cu', $request->id_cu)->where('id_tp', $request->id_tp)->whereBetween('tanggal', [$start, $end])->count();
			}
		} else {
			if ($request->semua == true) {
				$count_temuan = MonitoringCu::where('id_cu', $request->id_cu)->orderBy('aspek', 'desc')->count();
			} else {
				$count_temuan = MonitoringCu::where('id_cu', $request->id_cu)->whereBetween('tanggal', [$start, $end])->count();
			}
		}

		return Excel::download(new ExportMonitoringCu($request->id_cu, $request->id_tp, $request->tgl_mulai, $request->tgl_akhir, $request->semua, $count_temuan,$summary), 'Laporan.xlsx');
	}

	public function downloadLaporanKonsolidasi(Request $request)
	{
		return Excel::download(new MonitoringKonsolidasiCU($request->tahun, $request->bulan), 'Laporan.xlsx');
	}

	public function getPeriode($kegiatan_tipe)
	{
		$table_data = MonitoringCu::selectRaw('YEAR(tanggal) as tahun')
			->distinct()
			->orderBy('tahun', 'desc')
			->pluck('tahun');
