...
class PublicController extends Controller
{
	...
	{
		$artikelsUtama = Artikel::with('kategori', 'penulis', 'Cu')->where('terbitkan', 1)->where('utamakan', 1)->where('id_cu', 0)->orderBy('created_at', 'desc')->take(10)->get();

		$artikelsUtamaIds = $artikelsUtama->pluck('id')->toArray();

		$artikelsBKCUNew = Artikel::with('kategori', 'penulis', 'Cu')->whereNotIn('id', $artikelsUtamaIds)->where('id_cu', 0)->where('terbitkan', 1)->where('utamakan', '!=', 1)->orderBy('created_at', 'desc')->get()->take(10);

		// $artikelsBKCUNew = ArtikelKategori::with(['hasartikel' => function($q) {
		// 	$q->orderBy('created_at', 'desc')
		// 		->with('penulis');
		// 	}])
		// 	->whereHas('hasartikel', function ($query) {
		// 			$query->where('terbitkan', 1)
		// 						->where('utamakan', 0);
		// 	})
		// 	->get()->take(4)
		// 	->map(function ($artikelKategori) {
		// 			$artikelKategori->setRelation('hasartikel', $artikelKategori->hasartikel->take(5));
		// 			return $artikelKategori;
		// 	});

		$artikelsCUNew = Artikel::with('kategori', 'penulis', 'Cu')->whereNotIn('id', $artikelsUtamaIds)->where('id_cu', '!=', 0)->where('terbitkan', 1)->orderBy('created_at', 'desc')->get()->take(8);
	...
	public function artikel()
	{
		...
		$artikelsUtama = Artikel::with('kategori', 'penulis', 'Cu')->where('terbitkan', 1)->where('utamakan', 1)->where('id_cu', 0)->orderBy('created_at', 'desc')->take(5)->get();
		
		$artikelsUtamaIds = $artikelsUtama->pluck('id')->toArray();
		
		$artikels = Artikel::with('kategori', 'penulis')->whereNotIn('id', $artikelsUtamaIds)->where('id_cu', 0)->where('terbitkan', 1)->orderBy('created_at', 'desc')->paginate(8);
		...
		return view('artikel', compact('title', 'subtitle', 'tipe', 'artikels','artikelsUtama'));
	}
	...
	public function artikelKategori($slug)
	{
		...
		$artikelsUtama = Artikel::with('kategori', 'penulis', 'Cu')->where('id_artikel_kategori', $kategori->id)->where('terbitkan', 1)->where('utamakan', 1)->where('id_cu', 0)->orderBy('created_at', 'desc')->take(5)->get();

		$artikelsUtamaIds = $artikelsUtama->pluck('id')->toArray();

		$artikels = Artikel::with('penulis')->whereNotIn('id', $artikelsUtamaIds)->where('id_cu', 0)->where('id_artikel_kategori', $kategori->id)->where('terbitkan', 1)->orderBy('created_at', 'desc')->paginate(8);
	...
	public function artikelPenulis($slug)
	{
		...
		$artikelsUtama = Artikel::with('kategori', 'penulis', 'Cu')->where('id_artikel_penulis', $penulis->id)->where('terbitkan', 1)->where('utamakan', 1)->where('id_cu', 0)->orderBy('created_at', 'desc')->take(5)->get();
		
		$artikelsUtamaIds = $artikelsUtama->pluck('id')->toArray();

		$artikels = Artikel::with('kategori')->whereNotIn('id', $artikelsUtamaIds)->where('id_cu', 0)->where('id_artikel_penulis', $penulis->id)->where('terbitkan', 1)->orderBy('created_at', 'desc')->paginate(8);
		...
		return view('artikel', compact('title', 'subtitle', 'tipe', 'artikels', 'penulis','artikelsUtama'));
	}
	...
	public function artikelCari()
	{
		...
		$artikels = Artikel::with('kategori', 'penulis')->where('id_cu', 0)->where('terbitkan', 1)->where('name', 'like', '%' . $cari . '%')->orderBy('created_at', 'desc')->paginate(8);

		$q_anggotaCuCuueries['cari_column'] = 'name';
		$q_anggotaCuCuueries['cari'] = $cari;
		3;
	...
	public function cu()
	{
		...
		$cus = Cu::with('provinces')->orderBy('no_ba')->get();
	...
	public function dokumen()
	{
		...
		$dokumens = Dokumen::where('status', 'PUBLIK')
			->where('id_cu', 0)
			->get();
	...
	public function iuran()
	{
		...
		$kelas = \App\Models\JalinanIuran::create([
			'id_cu' => $cu,
			'periode' => $dateString,
			'status' => '0'
		]);

		$produk_data = \App\Models\ProdukCu::where('id_cu', $cu)->select('id', 'id_cu', 'name', 'tipe', 'jalinan')->where('jalinan', 1)->get();
		...
		$anggota_data = \App\Models\AnggotaCuCu::with('anggota_cu_simple', 'anggota_produk_cu.transaksi')->where('cu_id', $cu)
					...
					foreach ($q_anggotaCuCu->anggota_produk_cu as $q_anggotaProdukCu) {
						foreach ($produk_data as $produk) {
							...
							if ($produk->id == $q_anggotaProdukCu->produk_cu_id) {
								...
								\App\Models\JalinanIuranProduk::create([
	...
	{
		\App\Models\JalinanIuranAnggota::create([
			'jalinan_iuran_id' => $kelas->id,
	...
	{
		$kelas = \App\Models\AnggotaCu::whereIn('nik', [
			'6109053012720001',
			'6109052508430001',
		])->select('id', 'nik')->get();

		foreach ($kelas as $k) {
			$kelas2 = \App\Models\AnggotaCuCu::where('anggota_cu_id', $k->id)->whereNull('tanggal_keluar')->first();
			$kelas2->tanggal_keluar = '2021-09-21';
			$kelas2->keterangan_keluar = 'mutasi anggota';
			$kelas2->update();

			\App\Models\AnggotaCuCu::create([
				'anggota_cu_id' => $k->id,
				'cu_id' => '41',
				'tp_id' => '132',
				'no_ba' => $kelas2->no_ba,
	...
	public function testroute()
	{
		...
		// $kelas = \App\Models\JalinanIuranAnggota::with('anggota_cu','anggota_cu_cu')->where('jalinan_iuran_id',1)->where('lokasi', 5)->get();
