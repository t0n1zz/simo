...
use App\Models\KegiatanMateriNilai;
  ...
  public function updateNomorSertifikatKegiatan($id, Request $request)
{
    ...
    try {
        ...
        $idPesertaBaru = $pesertaBaru->filter(function ($item) {
  ...
  public function generateSertifikat(Request $formData)
  {
    ...
    $tahun_buku = $kegiatanData->tahun_buku;
    ...
    if($formData->peran === 'peserta'){ 
      ...
      if($sertifikat->tipe == 'diklat'){
       ...
       try {
            ...
            $pdf_doc = PDF::loadView('sertifikatRat', compact('backgroundMateriPath','tema','imagePath','nama', 'jabatan', 'lembaga', 'kegiatan', 'hari', 'tahun_buku', 'tgl', 'nomor', 'imageDepan', 'listMateri', 'sumWaktu', 'tempat', 'tglGenerate', 'imageBelakang'));
            $pdf_doc->setPaper($customPaper, $paperOrientation);
            return $pdf_doc->download('sertifikat_' . $kegiatan . '_' . $nama . '.pdf');
          } catch (\Exception $e) {
              return response()->json(['error' => 'Gagal membuat PDF: ' . $e->getMessage()], 500);
