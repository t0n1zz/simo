  ...
  <div>
      ...
      <div class="card-header d-print-none" v-if="!isNoButtonRow">
        <div class="row">
          ...
          <div class="col-md-5 col-lg-3 text-right d-none d-sm-block">
            <button type="button" class="btn bg-blue-300 btn-icon mb-1 mr-1" :disabled="itemDataStat === 'loading'" @click.prevent.prevent="modalOptionOpen('column')" v-if="!isNoKolom">
              <i class="icon-table2"></i> Kolom
            </button>
            <button type="button" class="btn bg-green-300 btn-icon mb-1 mr-1" :disabled="itemDataStat === 'loading'" @click.prevent.prevent="modalOptionOpen('excel')" v-if="!isNoExcel">
        ...
        <table class="table table-striped">
          ...
          <thead class="bg-primary" style="position: sticky; top: 0; z-index: 10;">
            <tr class="text-nowrap">
              <template v-for="column in columnData" :key="column.name">
                <th v-if="column && !column.hide && !column.disable">
                  <span v-html="column.title"></span> <i class="icon-menu-open" v-if="column.name == query.order_column"></i>
                </th>
              </template>
            </tr>
          </thead>
          ...
          <tbody v-else-if="itemDataStat === 'success'">
            ...
            <tr v-if="!itemData || !itemData.data || itemData.data.length == 0">
              <td :colspan="columnData.length">
                Belum terdapat data...
              </td>
            </tr>

            <template v-else v-for="(item,index) in itemData.data" :key="index">
              <slot name="item-desktop" :item="item" :index="index"></slot>
            </template>
    ...
    <div v-if="this.dataview == 'grid'">
      ...
      <div v-else-if="itemDataStat === 'success'">
        ...
        <div v-if="!itemData || !itemData.data || itemData.data.length == 0">
          <div class="card">
            <div class="card-body">
      ...
      <template #modal-body1>
        <div>
          ...
          <div v-if="modalOptionState === 'column'">
            ...
            <!-- column button -->
            <template v-for="(column,index) in columnData" :key="column.name">
              <button class="btn btn-block mb-1" v-if="column.hide != null && !column.disable" :class="{'btn-primary' : !column.hide}" @click.prevent.prevent="hideColumn(index)">{{column.title}}</button>
            </template>
            <hr/>
          ...
          <div v-else-if="modalOptionState === 'excel'">
            ...
            <!-- uploadexcel -->
            <template v-for="(ex, index) in excelUploads" :key="index">
              <button class="btn btn-light btn-block mb-1" v-if="ex.enabled" @click.prevent.prevent="modalExcelUploadOpen(index)"><i class="icon-file-upload"></i> {{ ex.button }}</button>
            </template>
        ...
        <div class="text-center">
          ...
          <div v-if="state === 'excel'">
            ...
            <ExcelButton
              ...
              :name="title + '.xlsx'"
            >
              <i class="icon-file-download"></i> Download data di tabel
            </ExcelButton> 
          </div>
          ...
          <div v-else-if="state === 'excelAll'">
            ...
            <div v-else-if="excelAllDataStat === 'success'">
              ...
              <ExcelButton
                ...
                :name="title + '.xlsx'"
              >
                <i class="icon-folder-download2"></i> Download Excel
              </ExcelButton>   
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      downloadFormatExcel(index){
        ...
        responseType: 'blob'})
        .then(response => {
          saveAs(response.data, this.excelUploads[index].format_url)
        });
      },
