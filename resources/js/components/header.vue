<template>
	<div>
		...
		<div class="navbar navbar-expand-lg navbar-dark bg-indigo">
			...
			<div class="d-lg-none">
				<button class="navbar-toggler" type="button" @click="navbarFirstOpen = !navbarFirstOpen">
					<i class="icon-tree5"></i>
				</button>
			</div>

			<div class="navbar-collapse navbar-menu" :class="{'is-open': navbarFirstOpen}" id="navbar-mobile">
		...
		<div class="navbar navbar-expand-lg navbar-light">
			<div class="text-center d-lg-none w-100">
				<button type="button" class="navbar-toggler dropdown-toggle" @click="navbarMenuOpen = !navbarMenuOpen">
					<i class="icon-unfold mr-2"></i>
					Menu
				</button>
			</div>

			<div class="navbar-collapse navbar-menu" :class="{'is-open': navbarMenuOpen}" id="navbar-navigation">
				...
				<ul class="navbar-nav navbar-nav-highlight">
					...
					<li class="nav-item dropdown" v-if="currentUser && currentUser.can['create_artikel'] || currentUser.can['create_artikel_kategori'] || currentUser.can['create_artikel_penulis'] || currentUser.can['index_artikel'] || currentUser.can['index_artikel_kategori'] || currentUser.can['index_artikel_penulis']" @mouseenter="showDropdown('publikasi')" @mouseleave="hideDropdown('publikasi')">
						...
						<div class="dropdown-menu" :class="{'show': activeDropdown === 'publikasi'}">
					...
					<li class="nav-item dropdown" @mouseenter="showDropdown('kegiatan')" @mouseleave="hideDropdown('kegiatan')">
						...
						<div class="dropdown-menu" :class="{'show': activeDropdown === 'kegiatan'}">
							...
							<div class="dropdown-submenu" v-if="currentUser && currentUser.can['create_diklat_bkcu'] || currentUser.can['create_voting'] || currentUser.can['create_tempat'] " @mouseenter="dropdown('tambahKegiatan')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'tambahKegiatan'}">
									...
									<!-- tambah diklat bkcu -->
									<router-link :to="{ name:'kegiatanBKCUCreate', params:{tipe: 'diklat_bkcu_internal'}  }" class="dropdown-item" active-class="active" exact v-if="currentUser.id_cu == 0">
										Diklat Internal PUSKOPCUINA
									</router-link>
							...
							<!-- diklat bkcu -->
							<router-link :to="{ name: 'kegiatanBKCU', params:{tipe: 'diklat_bkcu', periode: momentYear() }}" class="dropdown-item" active-class="active" v-if="currentUser.id_cu != 0"  exact>
								<i class="icon-graduation2"></i> Diklat PUSKOPCUINA
							</router-link>

							<!-- pertemuan bkcu -->
							<router-link :to="{ name: 'kegiatanBKCU', params:{tipe: 'pertemuan_bkcu', periode: momentYear() }}" class="dropdown-item" active-class="active" v-if="currentUser.id_cu != 0"  exact>
							...
							<div class="dropdown-submenu" v-if="currentUser.id_cu == 0 && currentUser.can['index_diklat_bkcu']" :class="{'show' : dropdownMenu2 == 'pesertaDiklat'}" @mouseenter="dropdown('pesertaDiklat')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'pesertaDiklat'}">
									...
									<router-link :to="{ name: 'kegiatanSemuaPeserta', params:{tipe: 'diklat_bkcu', cu: 'semua' }}" class="dropdown-item" active-class="active" exact >
										Diklat PUSKOPCUINA
									</router-link>
									
									<router-link :to="{ name: 'kegiatanSemuaPeserta', params:{tipe: 'pertemuan_bkcu', cu: 'semua' }}" class="dropdown-item" active-class="active" exact >
										Pertemuan PUSKOPCUINA
									</router-link>

									<router-link :to="{ name: 'kegiatanSemuaPeserta', params:{tipe: 'diklat_bkcu_internal', cu: 'semua' }}" class="dropdown-item" active-class="active" exact >
										Diklat Internal PUSKOPCUINA
									</router-link>
									
									<router-link :to="{ name: 'kegiatanSemuaPeserta', params:{tipe: 'pertemuan_bkcu_internal', cu: 'semua' }}" class="dropdown-item" active-class="active" exact >
										Pertemuan Internal PUSKOPCUINA
									</router-link>
							...
							<!-- divider -->
							<div class="dropdown-divider" v-if="currentUser.can['index_voting']"></div>
					...
					<li class="nav-item dropdown" v-if="currentUser && currentUser.can['create_anggota_cu'] || currentUser.can['create_jalinan_klaim'] || currentUser.can['index_anggota_cu'] || currentUser.can['index_saldo'] || currentUser.can['index_jalinan_klaim'] || currentUser.can['index_jalinan_iuran']" @mouseenter="showDropdown('anggotaCu')" @mouseleave="hideDropdown('anggotaCu')">
						...
						<div class="dropdown-menu" :class="{'show': activeDropdown === 'anggotaCu'}">
							...
							<!-- cari anggota cu -->
							<router-link :to="{ name: 'anggotaCuInformasi' }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['informasi_anggota_cu']">
								<i class="icon-folder-search"></i> Informasi Anggota CU
							</router-link>
					...
					<li class="nav-item dropdown" v-if="currentUser && currentUser.can['create_cu'] || currentUser.can['create_tp'] || currentUser.can['create_aktivis'] || currentUser.can['create_produk_cu'] || currentUser.can['create_mitra_orang'] ||  currentUser.can['create_mitra_lembaga'] || currentUser.can['create_pemilihan'] || currentUser.can['create_surat'] || currentUser.can['index_cu'] || currentUser.can['index_tp'] || currentUser.can['index_aktivis'] || currentUser.can['index_produk_cu'] || currentUser.can['index_mitra_orang'] || currentUser.can['index_pemilihan'] || currentUser.can['index_surat']" @mouseenter="showDropdown('organisasi')" @mouseleave="hideDropdown('organisasi')">
						...
						<div class="dropdown-menu" :class="{'show': activeDropdown === 'organisasi'}">
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_aktivis']" :class="{'show' : dropdownMenu2 == 'aktivis'}" @mouseenter="dropdown('aktivis')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'aktivis'}">
									...
									<router-link :to="{ name: 'aktivisCu', params:{cu: currentUser.id_cu, tingkat: 'magang'} }" class="dropdown-item" active-class="active" exact >
										Magang
									</router-link>
							...
							<div class="dropdown-submenu" :class="{'show' : dropdownMenu2 == 'dokumen'}" @mouseenter="dropdown('dokumen')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'dokumen'}">
							...
							<div class="dropdown-submenu" :class="{'show' : dropdownMenu2 == 'asetTetap'}" v-if="currentUser.can['index_aset_tetap'] || currentUser.can['index_aset_tetap_jenis'] || currentUser.can['index_aset_tetap_lokasi']" @mouseenter="dropdown('asetTetap')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'asetTetap'}">
									...
									<div class="dropdown-divider" v-if="currentUser.can['index_aset_tetap_jenis'] || currentUser.can['index_aset_tetap_lokasi']"></div>
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_mitra_orang'] || currentUser.can['index_mitra_lembaga']" :class="{'show' : dropdownMenu2 == 'mitra'}" @mouseenter="dropdown('mitra')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'mitra'}">
									...
									<!-- mitra perseorangan -->
									<router-link :to="{ name: 'mitraOrangCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_mitra_orang']">
										Perseorangan 
									</router-link>

									<!-- mitra lembaga -->
									<router-link :to="{ name: 'mitraLembagaCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_mitra_lembaga']">
										Lembaga 
									</router-link>
					...
					<li class="nav-item dropdown" v-if="currentUser && currentUser.can['create_laporan_cu'] || currentUser.can['create_assesment_access'] || currentUser.can['create_assesment_culeg'] || currentUser.can['create_monitoring'] || currentUser.can['index_laporan_cu'] || currentUser.can['index_assesment_access'] || currentUser.can['index_assesment_culeg'] || currentUser.can['index_monitoring'] || currentUser.can['index_monitoring_cu']" @mouseenter="showDropdown('tataKelola')" @mouseleave="hideDropdown('tataKelola')">
						...
						<div class="dropdown-menu" :class="{'show': activeDropdown === 'tataKelola'}">
						...
						<div class="dropdown-menu" :class="{'show': activeDropdown === 'pemberdayaan'}">
							...
							<div class="dropdown-submenu" v-if="currentUser && currentUser.can['create_kubn']" @mouseenter="dropdown('tambahPemberdayaan')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'tambahPemberdayaan'}">
									...
									<!-- tambah komunitas pemberdayaan -->
									<router-link :to="{ name:'kombasCreate' }" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_kombas']">
										Komunitas 
									</router-link>

									<!-- tambah enterpreneur -->
									<router-link :to="{ name:'enterpreneurCreate'}" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_enterpreneur']">
										Enterpreneur
									</router-link>

									<!-- tambah umkm -->
									<router-link :to="{ name:'umkmCreate'}" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_umkm']">
										Umkm
									</router-link>

									<!-- tambah mentor -->
									<router-link :to="{ name:'mentorCreate'}" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_mentor']">
										Mentor
									</router-link>

									<!-- tambah fasilitator -->
									<router-link :to="{ name:'fasilitatorCreate'}" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_fasilitator']">
										Fasilitator
									</router-link>

									<!-- tambah usaha  -->
									<router-link :to="{ name:'kubnUsahaCreate' }" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_usaha']">
										Usaha
									</router-link>

									<!-- tambah keahlian-->
									<router-link :to="{ name:'keahlianCreate' }" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_keahlian']">
										Keahlian
									</router-link>

									<!-- tambah jenis diklat -->
									<router-link :to="{ name:'jenisDiklatCreate' }" class="dropdown-item" active-class="active" exact v-if="currentUser.can['create_jenis_diklat']">
										Jenis Diklat
									</router-link>
							...
							<!-- divider -->
							<div class="dropdown-divider" v-if="currentUser.can['create_kubn']"></div> 
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_kubn'] && currentUser.id_cu == 0" :class="{'show' : dropdownMenu2 == 'kubn'}" @mouseenter="dropdown('kubn')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'kubn'}">
									...
									<router-link :to="{ name: 'kubnCu', params:{cu:'semua'} }" class="dropdown-item" active-class="active" exact >
										Semua CU
									</router-link>
									...
									<template v-for="(cu, index) in modelCu">
										<router-link :to="{ name: 'kubnCu', params:{cu: cu.id} }" class="dropdown-item" active-class="active" exact v-if="cu" :key="index">
											CU {{ cu.name }}
										</router-link>
									</template>
								</div>
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_kubn'] && currentUser.id_cu == 0" :class="{'show' : dropdownMenu2 == 'kombas'}" @mouseenter="dropdown('kombas')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'kombas'}">
									...
									<router-link :to="{ name: 'kombasTipe', params:{tipe: 'semua'} }" class="dropdown-item" active-class="active" exact >
										Semua CU
									</router-link>
									...
									<template v-for="(cu, index) in modelCu">
										<router-link :to="{ name: 'kombasCu', params:{cu: cu.id, tipe: 'semua'} }" class="dropdown-item" active-class="active" exact v-if="cu" :key="index">
											CU {{ cu.name }}
										</router-link>
									</template>
								</div>
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_enterpreneur'] && currentUser.id_cu == 0" :class="{'show' : dropdownMenu2 == 'enterpreneur'}" @mouseenter="dropdown('enterpreneur')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'enterpreneur'}">
									...
									<router-link :to="{ name: 'enterpreneurCu', params:{cu:'semua'} }" class="dropdown-item" active-class="active" exact >
										Semua CU
									</router-link>
									...
									<template v-for="(cu, index) in modelCu">
										<router-link :to="{ name: 'enterpreneurCu', params:{cu: cu.id} }" class="dropdown-item" active-class="active" exact v-if="cu" :key="index">
											CU {{ cu.name }}
										</router-link>
									</template>
								</div>
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_umkm'] && currentUser.id_cu == 0" :class="{'show' : dropdownMenu2 == 'umkm'}" @mouseenter="dropdown('umkm')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'umkm'}">
									...
									<router-link :to="{ name: 'umkmCu', params:{cu:'semua'} }" class="dropdown-item" active-class="active" exact >
										Semua CU
									</router-link>
									...
									<template v-for="(cu, index) in modelCu">
										<router-link :to="{ name: 'umkmCu', params:{cu: cu.id} }" class="dropdown-item" active-class="active" exact v-if="cu" :key="index">
											CU {{ cu.name }}
										</router-link>
									</template>
								</div>
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_mentor'] && currentUser.id_cu == 0" :class="{'show' : dropdownMenu2 == 'mentor'}" @mouseenter="dropdown('mentor')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'mentor'}">
									...
									<router-link :to="{ name: 'mentorCu', params:{cu:'semua'} }" class="dropdown-item" active-class="active" exact >
										Semua CU
									</router-link>
									...
									<template v-for="(cu, index) in modelCu">
										<router-link :to="{ name: 'mentorCu', params:{cu: cu.id} }" class="dropdown-item" active-class="active" exact v-if="cu" :key="index">
											CU {{ cu.name }}
										</router-link>
									</template>
								</div>
							...
							<div class="dropdown-submenu" v-if="currentUser.can['index_fasilitator'] && currentUser.id_cu == 0" :class="{'show' : dropdownMenu2 == 'fasilitator'}" @mouseenter="dropdown('fasilitator')" @mouseleave="dropdown('')">
								...
								<div class="dropdown-menu dropdown-scrollable" :class="{'show' : dropdownMenu2 == 'fasilitator'}">
									...
									<router-link :to="{ name: 'fasilitatorCu', params:{cu:'semua'} }" class="dropdown-item" active-class="active" exact >
										Semua CU
									</router-link>
									...
									<template v-for="(cu, index) in modelCu">
										<router-link :to="{ name: 'fasilitatorCu', params:{cu: cu.id} }" class="dropdown-item" active-class="active" exact v-if="cu" :key="index">
											CU {{ cu.name }}
										</router-link>
									</template>
								</div>
							...
							<router-link :to="{ name: 'kubnCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_kubn'] && currentUser.id_cu != 0">
								<i class="icon-versions"></i> KUBN
							</router-link>

							<router-link :to="{ name: 'kombasCu', params:{cu: currentUser.id_cu, tipe:'semua'} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_kubn'] && currentUser.id_cu != 0">
								<i class="icon-circles2"></i> Komunitas
							</router-link>

							<router-link :to="{ name: 'umkmCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_umkm'] && currentUser.id_cu != 0">
								<i class="icon-store2"></i> Umkm
							</router-link>

							<router-link :to="{ name: 'enterpreneurCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_enterpreneur'] && currentUser.id_cu != 0">
								<i class="icon-hat"></i> Enterpreneur
							</router-link>

							<router-link :to="{ name: 'mentorCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_mentor'] && currentUser.id_cu != 0">
								<i class="icon-brain"></i> Mentor
							</router-link>

							<router-link :to="{ name: 'fasilitatorCu', params:{cu: currentUser.id_cu} }" class="dropdown-item" active-class="active" exact v-if="currentUser && currentUser.can['index_fasilitator'] && currentUser.id_cu != 0">
								<i class="icon-unlink2"></i> Fasilitator
							</router-link>

							<!-- divider -->
							<div class="dropdown-divider" v-if="currentUser.can['index_kubn'] && currentUser.id_cu == 0"></div> 
							
							<!-- usaha -->
							<router-link :to="{ name: 'kubnUsaha' }" class="dropdown-item" active-class="active" v-if="currentUser.can['index_kubn'] && currentUser.id_cu == 0" exact>
								<i class="icon-price-tag2"></i> Usaha
							</router-link>

							<!-- keahlian -->
							<router-link :to="{ name: 'keahlian' }" class="dropdown-item" active-class="active" v-if="currentUser.can['index_keahlian'] && currentUser.id_cu == 0" exact>
								<i class="icon-reading"></i> Keahlian
							</router-link>

							<!-- jenis diklat -->
							<router-link :to="{ name: 'jenisDiklat' }" class="dropdown-item" active-class="active" v-if="currentUser.can['index_jenis_diklat'] && currentUser.id_cu == 0" exact>
								<i class="icon-book2"></i> Jenis Diklat
							</router-link>
				...
				<ul class="navbar-nav ml-lg-auto">
					...
					<li class="nav-item dropdown" v-if="currentUser && (currentUser.can['create_user'] || currentUser.can['index_user'])" @mouseenter="showDropdown('settings')" @mouseleave="hideDropdown('settings')">
						...
						<div class="dropdown-menu dropdown-menu-right" :class="{'show': activeDropdown === 'settings'}">
...
<style>
/* Custom navbar menu toggle for mobile */
...
navbarMenuOpen: false,
			...
			fetchTp(){
				if(this.currentUser.id_cu != '0'){
					if(this.modelTpStat != 'success'){
						const tpStore = useTpStore();
					tpStore.getCuHeader(this.currentUser.id_cu);
					}
				}
			},
			fetchCu(){
		...
		computed: {
			...
			isTokenExpired() {
				const authStore = useAuthStore();
				return authStore.getIsTokenExpired;
			},
			isFromLogin() {
				const authStore = useAuthStore();
				return authStore.getIsFromLogin;
			},
			notification() {
			...
			unreadNotification() {
				const notificationStore = useNotificationStore();
				return notificationStore.getUnreadNotification;
			},
			unreadNotificationStat() {
				const notificationStore = useNotificationStore();
				return notificationStore.getNotifStat;
			},
			markNotifStat() {
				const notificationStore = useNotificationStore();
				return notificationStore.getMarkNotifStat;
			},
			modelTp() {
				const tpStore = useTpStore();
				return tpStore.getHeaderDataS;
			},
			modelTpStat() {
				const tpStore = useTpStore();
				return tpStore.getHeaderDataStatS;
			},
			modelCu() {
				const cuStore = useCuStore();
				return cuStore.getHeaderDataS;
			},
			modelCuStat() {
				const cuStore = useCuStore();
				return cuStore.getHeaderDataStatS;
			},
			modelKegiatan() {
				const kegiatanBKCUStore = useKegiatanBKCUStore();
				return kegiatanBKCUStore.getDataJalan;
			},
			modelKegiatanStat() {
				const kegiatanBKCUStore = useKegiatanBKCUStore();
				return kegiatanBKCUStore.getDataJalanStat;
			},
			updateSaranResponse() {
				const saranStore = useSaranStore();
				return saranStore.getUpdate;
			},
			updateSaranStat() {
				const saranStore = useSaranStore();
				return saranStore.getUpdateStat;
			},
		}
	}
</script>
