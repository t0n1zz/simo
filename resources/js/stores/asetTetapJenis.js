...
import AsetTetapJenisAPI from '../api/asetTetapJenis';

export const useAsetTetapJenisStore = defineStore('asetTetapJenis', {
  state: () => ({
    data: {}, //single data
    dataS: [], //collection
    dataStat: '',
  ...
  actions: {
    ...
    async index(p) {
      ...
      try {
        const response = await AsetTetapJenisAPI.index(p);
    ...
    async create() {
      ...
      try {
        const response = await AsetTetapJenisAPI.create();
    ...
    async get(id) {
      ...
      try {
        const response = await AsetTetapJenisAPI.get(id);
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async edit(id) {
      ...
      try {
        const response = await AsetTetapJenisAPI.edit(id);
        this.data = response.data.form;
        this.rules = response.data.rules;
        this.options = response.data.options;
        this.dataStat = 'success';
