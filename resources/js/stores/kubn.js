...
import KubnAPI from '../api/kubn.js';

export const useKubnStore = defineStore('kubn', {
  state: () => ({
    data: {}, //single data
    dataS: [], //collection
    dataS2: [], //collection
  ...
  actions: {
    ...
    async index(p) {
      ...
      try {
        const response = await KubnAPI.index(p);
    ...
    async indexCu([p, id]) {
      ...
      try {
        const response = await KubnAPI.indexCu(p, id);
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async indexAnggota(id) {
      this.dataStatS2 = 'loading';

      try {
        const response = await KubnAPI.indexAnggota(id);
    ...
    async indexDiklat(id) {
      ...
      try {
        const response = await KubnAPI.indexDiklat(id);
    ...
    async get() {
      ...
      try {
        const response = await KubnAPI.get();
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async getCu(id) {
      ...
      try {
        const response = await KubnAPI.getCu(id);
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async create() {
      ...
      try {
        const response = await KubnAPI.create();
    ...
    async store(form) {
      ...
      try {
        const response = await KubnAPI.store(form);
    ...
    async edit(id) {
      ...
      try {
        const response = await KubnAPI.edit(id);
        this.data = response.data.form;
        this.rules = response.data.rules;
        this.options = response.data.options;
        this.dataStat = 'success';
    ...
    async update([id, form]) {
      ...
      try {
        const response = await KubnAPI.update(id, form);
        if (response.data.saved) {
          this.updateData = response.data;
          this.updateStat = 'success';
        } else {
    ...
    async destroy(id) {
      ...
      try {
        const response = await KubnAPI.destroy(id);
