...
export const useMitraLembagaStore = defineStore('mitraLembaga', {
  state: () => ({
    data: {}, //single data
    dataS: [], //collection
  ...
  actions: {
    ...
    async index(p) {
      ...
      try {
        const response = await MitraLembagaAPI.index(p);
    ...
    async indexCu([p, id]) {
      ...
      try {
        const response = await MitraLembagaAPI.indexCu(p, id);
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async create() {
      ...
      try {
        const response = await MitraLembagaAPI.create();
    ...
    async edit(id) {
      ...
      try {
        const response = await MitraLembagaAPI.edit(id);
        this.data = response.data.form;
        this.rules = response.data.rules;
        this.options = response.data.options;
        this.dataStat = 'success';
    ...
    async count() {
      ...
      try {
        const response = await MitraLembagaAPI.count();
