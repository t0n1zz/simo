...
export const useMitraOrangStore = defineStore('mitraOrang', {
  state: () => ({
    data: {}, //single data
    dataS: [], //collection
  ...
  actions: {
    ...
    async index(p) {
      ...
      try {
        const response = await MitraOrangAPI.index(p);
    ...
    async indexCu([p, id]) {
      ...
      try {
        const response = await MitraOrangAPI.indexCu(p, id);
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async indexPeserta([p, kegiatan_id]) {
      this.dataStatS = 'loading';

      try {
        const response = await MitraOrangAPI.indexPeserta(p, kegiatan_id);
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async create() {
      ...
      try {
        const response = await MitraOrangAPI.create();
    ...
    async edit(id) {
      ...
      try {
        const response = await MitraOrangAPI.edit(id);
        this.data = response.data.form;
        this.rules = response.data.rules;
        this.options = response.data.options;
        this.dataStat = 'success';
    ...
    async count() {
      ...
      try {
        const response = await MitraOrangAPI.count();
