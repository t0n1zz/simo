...
import PekerjaanAPI from '../api/pekerjaan.js';
...
export const usePekerjaanStore = defineStore('pekerjaan', {
    ...
    actions: {
        ...
        async index(p) {
            ...
            try {
                const response = await PekerjaanAPI.index(p);
        ...
        async get() {
            this.dataStatS = 'loading';
            try {
                const response = await PekerjaanAPI.get();
                this.dataS = response.data.model;
                this.dataStatS = 'success';
            } catch (error) {
                this.dataS = error.response;
        ...
        async create() {
            ...
            try {
                const response = await PekerjaanAPI.create();
        ...
        async store(form) {
            ...
            try {
                const response = await PekerjaanAPI.store(form);
        ...
        async edit(id) {
            this.dataStat = 'loading';
            try {
                const response = await PekerjaanAPI.edit(id);
                this.data = response.data.form;
                this.rules = response.data.rules;
                this.options = response.data.options;
                this.dataStat = 'success';
        ...
        async update([id, form]) {
            this.updateStat = 'loading';
            try {
                const response = await PekerjaanAPI.update(id, form);
                if (response.data.saved) {
                    this.updateData = response.data;
                    this.updateStat = 'success';
                } else {
        ...
        async destroy(id) {
            this.updateStat = 'loading';
            try {
                const response = await PekerjaanAPI.destroy(id);
