...
import SukuAPI from '../api/suku.js';

export const useSukuStore = defineStore('suku', {
  state: () => ({
    data: {}, //single data
    dataS: [], //collection
    dataStat: '',
  ...
  actions: {
    ...
    async index(p) {
      ...
      try {
        const response = await SukuAPI.index(p);
    ...
    async get() {
      ...
      try {
        const response = await SukuAPI.get();
        this.dataS = response.data.model;
        this.dataStatS = 'success';
      } catch (error) {
        this.dataS = error.response;
    ...
    async create() {
      ...
      try {
        const response = await SukuAPI.create();
    ...
    async store(form) {
      ...
      try {
        const response = await SukuAPI.store(form);
    ...
    async edit(id) {
      ...
      try {
        const response = await SukuAPI.edit(id);
        this.data = response.data.form;
        this.rules = response.data.rules;
        this.options = response.data.options;
        this.dataStat = 'success';
    ...
    async update([id, form]) {
      ...
      try {
        const response = await SukuAPI.update(id, form);
        if (response.data.saved) {
          this.updateData = response.data;
          this.updateStat = 'success';
        } else {
    ...
    async destroy(id) {
      ...
      try {
        const response = await SukuAPI.destroy(id);
