	...
	export default {
		...
		methods: {
			fetch(){
				...
				this.aktivisStore.editIdentitas(this.id_local);	
			...
			changeTab(value){
				...
				if(value == 'identitas'){
					...
					this.aktivisStore.indexPekerjaan(this.id_local);	
				}else if(value =='riwayatPendidikan'){
					this.aktivisStore.indexPendidikan(this.id_local);
				}else if(value =='riwayatOrganisasi'){
					this.aktivisStore.indexOrganisasi(this.id_local);
				}else if(value =='keluarga'){
					this.aktivisStore.indexKeluarga(this.id_local);
				}else if(value =='anggotaCu'){
					this.aktivisStore.indexAnggotaCu(this.id_local);
				}
			},
			back(){
			...
			createRiwayat(){
				...
				if(this.tabName == 'riwayatPekerjaan'){
					...
					this.aktivisStore.createPekerjaan();	
				}else if(this.tabName == 'riwayatPendidikan'){
					this.modalTitle = 'Tambah riwayat pendidikan';
					if(this.modelCuStat != 'success'){
						this.cuStore.getHeader();
					}
					this.aktivisStore.createPendidikan();
				}else if(this.tabName == 'riwayatOrganisasi'){
					this.modalTitle = 'Tambah riwayat organisasi';
					if(this.modelCuStat != 'success'){
						this.cuStore.getHeader();
					}
					this.aktivisStore.createOrganisasi();
				}else if(this.tabName == 'keluarga'){
					this.modalTitle = 'Tambah data keluarga';
					if(this.modelCuStat != 'success'){
						this.cuStore.getHeader();
					}
					this.aktivisStore.createKeluarga();
				}else if(this.tabName == 'anggotaCu'){
					this.modalTitle = 'Tambah data anggota CU';
					if(this.modelCuStat != 'success'){
						this.cuStore.getHeader();
					}
					this.aktivisStore.createAnggotaCu();
				}
			},
			updateRiwayat(data){
				this.modalShow = true;
			...
			modalConfirmOk() {
				if (this.tabName == 'riwayatPekerjaan') {
					...
					this.aktivisStore.destroyKeluarga(this.selectedItemKeluarga.id);
				}else if (this.tabName == 'anggotaCu') {
					this.aktivisStore.destroyAnggotaCu(this.selectedItemAnggotaCu.id);
				}
			},
			cancelClick(){
				this.modalShow = false;
		...
		computed:{
			...
			rules() {
				return this.aktivisStore.rules;
			},
			options() {
