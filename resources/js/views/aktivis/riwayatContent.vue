			...
			<data-table :items="itemDataPekerjaan" :columnData="columnDataPekerjaan" :itemDataStat="itemDataStatPekerjaan">
				<template #item-desktop="props">
					<tr :class="{ 'bg-info': selectedItemPekerjaan.id === props.item.id }" class="text-nowrap" @click="selectedRowPekerjaan(props.item)" v-if="props.item">
						...
						<td v-html="$filters.checkTingkatAktivis(props.item.tingkat)"></td>
						<td>
						...
						<td v-html="$filters.date(props.item.mulai)"></td>
						<td>
							<span v-if="props.item.selesai">
								<span v-html="$filters.date(props.item.selesai)"></span>
							</span>
							<span v-else>-</span>
						</td>
						<td><check-value :value="props.item.keterangan_tidak_aktif"></check-value></td>
						<td v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
						<td>
			...
			<data-table :items="itemDataDiklat" :columnData="columnDataDiklat" :itemDataStat="itemDataStatDiklat">
				<template #item-desktop="props">
					<tr :class="{ 'bg-info': selectedItemDiklat.id === props.item.id }" class="text-nowrap" @click="selectedRowDiklat(props.item)" v-if="props.item">
						...
						<td>
							<span v-html="$filters.kegiatanTipe(props.item.kegiatan_tipe)"></span>
						</td>
						<td>
							<span v-if="props.item.kegiatan">
						...
						<td>
							<span v-if="props.item.kegiatan">
								<span v-html="$filters.date(props.item.kegiatan.mulai)" v-if="props.item.kegiatan"></span>
								<span v-else>-</span>
							</span>		
							<span v-else-if="props.item.kegiatan_id == 0">
								<span v-html="$filters.date(props.item.datang)" v-if="props.item.datang"></span>
								<span v-else>-</span>
							</span>
							<span v-else>-</span>
						</td>
						<td>
							<span v-if="props.item.kegiatan">
								<span v-html="$filters.date(props.item.kegiatan.selesai)" v-if="props.item.kegiatan"></span>
								<span v-else>-</span>
							</span>		
							<span v-else-if="props.item.kegiatan_id == 0">
								<span v-html="$filters.date(props.item.pulang)" v-if="props.item.pulang"></span>
								<span v-else>-</span>
							</span>
							<span v-else>-</span>	
						</td>
			...
			<data-table :items="itemDataKeterangan" :columnData="columnDataKeterangan" :itemDataStat="itemDataStatKeterangan">
				<template #item-desktop="props">
					<tr :class="{ 'bg-info': selectedItemKeterangan.id === props.item.id }" class="text-nowrap" @click="selectedRowKeterangan(props.item)" v-if="props.item">
						...
						<td v-html="$filters.date(props.item.tanggal)"></td>
						<td v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
						<td>
							<span v-if="props.item.created_at !== props.item.updated_at" v-html="$filters.dateTime(props.item.updated_at)"></span>
							<span v-else>-</span>
...
<script>
	...
	export default {
			...
			updateStat(value){
				...
				if(value === "success"){
					...
					if(this.formState == 'create pekerjaan' || this.formState == 'edit pekerjaan'){
						this.aktivisStore.indexPekerjaan([this.id_aktivis, this.id_cu]);
					}

					if(this.formState == 'create pendidikan' || this.formState == 'edit pendidikan'){
						this.aktivisStore.indexPendidikan([this.id_aktivis, this.id_cu]);
					}

					if(this.formState == 'create organisasi' || this.formState == 'edit organisasi'){
						this.aktivisStore.indexOrganisasi([this.id_aktivis, this.id_cu]);
					}

					if(this.formState == 'create diklat' || this.formState == 'edit diklat'){
						this.aktivisStore.indexDiklat([this.id_aktivis, this.id_cu]);
					}

					if(this.formState == 'create keterangan' || this.formState == 'edit keterangan'){
						this.aktivisStore.indexKeterangan([this.id_aktivis, this.id_cu]);
					}
				}else{
		...
		methods: {
			...
			modalConfirmOk() {
				if(this.formState == 'pekerjaan'){
					this.aktivisStore.destroyPekerjaan(this.selectedItemPekerjaan.id);
				}else if(this.formState == 'pendidikan'){
					this.aktivisStore.destroyPendidikan(this.selectedItemPendidikan.id);
				}else if(this.formState == 'organisasi'){
					this.aktivisStore.destroyOrganisasi(this.selectedItemOrganisasi.id);
				}else if(this.formState == 'diklat'){
					this.aktivisStore.destroyDiklat(this.selectedItemDiklat.id);
				}else if(this.formState == 'keterangan'){
					this.aktivisStore.destroyKeterangan(this.selectedItemKeterangan.id);
				}
			},
			cancelClick(){
				this.modalShow = false;
		...
		computed:{
			...
			form() {
				return this.aktivisStore.data;
			},
			formStat() {
				return this.aktivisStore.dataStat;
			},
			itemDataPekerjaan() {
				return this.aktivisStore.dataS1;
			},
			itemDataPendidikan() {
				return this.aktivisStore.dataS2;
			},
			itemDataOrganisasi() {
				return this.aktivisStore.dataS3;
			},
			itemDataDiklat() {
				return this.aktivisStore.dataS4;
			},
			itemDataKeterangan() {
				return this.aktivisStore.dataS5;
			},
			itemDataStatPekerjaan() {
				return this.aktivisStore.dataStatS1;
			},
			itemDataStatPendidikan() {
				return this.aktivisStore.dataStatS2;
			},
			itemDataStatOrganisasi() {
				return this.aktivisStore.dataStatS3;
			},
			itemDataStatDiklat() {
				return this.aktivisStore.dataStatS4;
			},
			itemDataStatKeterangan() {
