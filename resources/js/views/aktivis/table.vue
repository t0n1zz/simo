		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[24].hide" v-html="$filters.date(props.item.tanggal_lahir)">
					</td>
					...
					<td v-if="!columnData[31].hide" v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
      ...
      updateStat(value) {
		...
		methods: {
			fetch(params){
				...
				if(this.$route.params.cu == 'semua'){
					...
					this.aktivisStore.index([params, this.$route.params.tingkat, this.status]);
					...
					this.aktivisStore.indexCu([params,this.$route.params.cu, this.$route.params.tingkat, this.status]);
        ...
        this.modalContent = '/images/aktivis/' + item + '.jpg';
			...
			modalTutup() {
				...
				this.aktivisStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.aktivisStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
