					...
					<data-table :items="itemDataCu" :columnData="columnDataCu" :itemDataStat="itemDataCuStat">
						<template #item-desktop="props">
							<tr :class="{ 'bg-info': selectedItemCu.index === props.index + 1 }" class="text-nowrap" @click="selectedCuRow(props.index,props.item)" v-if="props.item">
								...
								<td v-html="$filters.date(props.item.tanggal_keluar)" class="text-nowrap"></td>
							</tr>
						</template>	
					</data-table>
...
<script>
	...
	import { usePekerjaanStore } from '../../stores/pekerjaan';
	import { useSukuStore } from '../../stores/suku';
	...
	export default {
		...
		methods: {
			...mapActions(useAnggotaCuStore, ['create', 'edit', 'store', 'update', 'resetUpdateStat']),
			...mapActions(useCuStore, ['getHeader']),
			...mapActions(useTpStore, { getTpCu: 'getCu' }),
			...mapActions(usePekerjaanStore, { getPekerjaan: 'get' }),
			...mapActions(useSukuStore, { getSuku: 'get' }),
			...mapActions(useProvincesStore, { getProvinces: 'get' }),
			...mapActions(useRegenciesStore, { getRegenciesInProvinces: 'indexProvinces' }),
			...mapActions(useDistrictsStore, { getDistrictsInRegencies: 'indexRegencies' }),
			...mapActions(useVillagesStore, { getVillagesInDistricts: 'indexDistricts' }),
			fetch() {
			...
			changeRegencies(id){
				this.getDistrictsInRegencies(id);
			},
			changeDistricts(id){
				this.getVillagesInDistricts(id);
			},
			back() {
