					...
					<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" @fetch="fetch">
						...
						<template #item-desktop="props">
							<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
								...
								<td v-if="!columnData[17].hide" v-html="$filters.date(props.item.tanggal_lahir)">
								</td>
								<td v-if="!columnData[18].hide">
									<check-value :value="props.item.tempat_lahir"></check-value>
								</td>
								<td v-if="!columnData[19].hide" v-html="$filters.date(props.item.tanggal_masuk)">
								</td>
...
<script>
	...
	export default {
      ...
      updateStat(value) {
		...
		methods: {
			...mapActions(useAnggotaCuStore, ['indexCuDraft', 'storeDraft', 'destroyDraft', 'destroyDraftAll', 'storeDraftAll', 'resetUpdateStat']),
			fetch(){
				this.indexCuDraft([this.query, this.currentUser.id]);
			},
			selectedRow(item) {
			...
			modalConfirmOk() {
				if (this.state == "simpan") {
					...
					this.destroyDraft(this.selectedItem.id);
				}else if (this.state == "hapus_semua") {
					this.destroyDraftAll(this.currentUser.id);
				}else if (this.state == "simpan") {
					this.storeDraftAll(this.currentUser.id);
				}
			}
		},
		computed: {
