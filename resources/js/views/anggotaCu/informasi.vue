          ...
          <div v-if="itemDataStat == 'success'">
                    ...
                    <div class="row">
                      <div class="col-sm-6">
                        <ul class="list list-unstyled mb-0">
                          ...
                          <li><b>Usia:</b> <span v-if="itemData.tanggal_lahir" v-html="$filters.age(itemData.tanggal_lahir)"></span></li>
            ...
            <div class="card">
              ...
              <data-table :items="itemDataCu" :columnData="columnDataCu" :itemDataStat="itemDataCuStat">
                <template #item-desktop="props">
                  <tr class="text-nowrap" v-if="props.item.escete == 1">
                    <!-- <td>
                    ...
                    <td v-html="$filters.date(props.item.tanggal_masuk)" class="text-nowrap"></td>
                    <td><check-value :value="props.item.keterangan_keluar"></check-value></td>
                    <td class="text-nowrap">
                      <span v-if="props.item.tanggal_keluar" v-html="$filters.dateTime(props.item.tanggal_keluar)"></span>
						          <span v-else>-</span>
                    </td>
                  </tr>
                </template>	
            ...
            <div class="card">
              ...
              <data-table :items="itemDataProduk" :columnData="columnDataProduk" :itemDataStat="itemDataProdukStat">
                <template #item-desktop="props">
                  <tr class="text-nowrap" v-if="props.item.cu && props.item.cu.escete == 1">
                    <td>{{ props.index + 1 }}</td>
                    <td>
                      <check-value :value="props.item.cu.name" v-if="props.item.cu"></check-value>
                      <span v-else>-</span>
                    ...
                    <td>
                      <span v-if="props.item.cu.tanggal_update_data" v-html="$filters.date(props.item.cu.tanggal_update_data)"></span>
                      <span v-else>-</span>
                    </td>
                    <td class="text-nowrap">
                      <span v-if="props.item.pivot.tanggal" v-html="$filters.date(props.item.pivot.tanggal)"></span>
						          <span v-else>-</span>
                    </td>
                  </tr>
                </template>	
        ...
        <data-table :items="itemDataUpdateESCETE" :columnData="columnDataUpdateESCETE" :itemDataStat="itemDataUpdateESCETEStat">
          <template #item-desktop="props">
            <tr class="text-nowrap" v-if="props.item">
              ...
              <td>
                <span v-if="props.item.tanggal_update_data" v-html="$filters.date(props.item.tanggal_update_data)"></span>
                <span v-else>-</span>
              </td>
            </tr>
          </template>	
...
<script>
  ...
  export default {
    ...
    methods: {
			...mapActions(useAnggotaCuStore, ['cariDataInformasi', 'setData', 'setDataStat']),
			...mapActions(useCuStore, ['getUpdateESCETE']),
      cariData(){
        if(this.nik.length < 16){
          ...
          this.isNew = true;
          this.isDigit = false;
          this.cariDataInformasi(this.nik);
        }
      },
      fetchCu(){
        this.itemDataCu = [];
