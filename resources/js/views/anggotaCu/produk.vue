						...
						<div class="card-body">
							...
							<div class="card card-body text-center" v-for="(item, index) in form.anggota_cu_cu" :key="index">
								<ul class="list-inline list-inline-consensed mb-0 font-size-lg">
									...
									<li class="list-inline-item"><b>Tgl. Masuk:</b> {{ $filters.date(item.tanggal_masuk) }}</li>
									<li class="list-inline-item" v-if="item.tanggal_keluar"><b>Tgl. Keluar:</b> {{ $filters.date(item.tanggal_keluar) }}</li>
								</ul>
							</div>
						...
						<data-table :items="itemData" :columnData="columnData" :itemDataStat="itemDataStat">
							<template #item-desktop="props">
								<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)" v-if="props.item">
									...
									<td>
										<span v-if="props.item.tanggal_target" v-html="$filters.date(props.item.tanggal_target)"></span>
										<span v-else>-</span>
									</td>
									<td>
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions(useAnggotaCuStore, ['indexProduk', 'edit', 'destroyProduk']),
			...mapActions(useCuStore, ['getHeader']),
			fetch(){
				this.indexProduk([this.$route.params.id, this.$route.params.cu]);
				this.edit(this.$route.params.id, this.$route.params.cu);
			},
			back(){
			...
			modalConfirmOk() {
				this.destroyProduk(this.selectedItem.id);
			},
			cancelClick(){
				this.modalShow = false;
			},
