<template>
  <div>
    ...
    <data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :excelUploads="excelUploads"  @fetch="fetch">
      ...
      <template #item-desktop="props">
          ...
          <td v-if="!columnData[24].hide">
					...
					<td v-if="!columnData[29].hide" v-html="$filters.date(props.item.tanggal_lahir)">
					</td>
					<td v-if="!columnData[30].hide">
						<check-value :value="props.item.tempat_lahir"></check-value>
					</td>
          <td v-if="!columnData[31].hide">
            <span v-html="$filters.date(props.item.tanggal_masuk)"></span>
					</td>
					...
					<td v-if="!columnData[39].hide" v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
    ...
    <app-modal :show="modalShow" :state="modalState" :title="modalTitle" :button="modalButton" :content="modalContent" :color="modalColor" 
      ...
      <!-- title -->
			<template #modal-title v-if="modalState != 'normal1'">
				{{ modalTitle }}
			</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(useAnggotaCuStore, [
        'index', 'indexKeluar', 'indexMeninggal', 'indexCu', 'indexCuKeluar', 'indexCuMeninggal', 'destroy', 'updateBatalKeluar', 'resetUpdateStat'
      ]),
      ...
      fetchCu(){
        this.escete(this.$route.params.cu);
      },
      fetchAnggotaCuDraft(){
				let cu = '';
				let tp = 'semua';
      ...
      modalConfirmOk() {
        if (this.state == "hapus") {
          if(this.$route.params.cu != 'semua'){
            this.destroy([this.selectedItem.id, this.$route.params.cu]);
          }else{
            // todo: add selection delete
          }
        }else if(this.state == "keluar"){
          this.updateBatalKeluar(this.selectedItem.id);
        }
      }
    },
    computed: {
