    ...
    <data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :excelDownloadUrl="excelDownloadUrl" :itemDataStat="itemDataStat" @fetch="fetch">
      ...
      <template #item-desktop="props">
          ...
          <td v-if="!columnData[17].hide">
					...
					<td v-if="!columnData[22].hide" v-html="$filters.date(props.item.tanggal_lahir)">
					</td>
					<td v-if="!columnData[23].hide">
						<check-value :value="props.item.tempat_lahir"></check-value>
					</td>
          <td v-if="!columnData[24].hide" v-html="$filters.date(props.item.tanggal_masuk)">
					</td>
          ...
          <td v-if="!columnData[29].hide">
					...
					<td v-if="!columnData[32].hide" v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions(useAnggotaCuStore, ['indexCuDraft', 'resetUpdateStat', 'storeDraft', 'destroyDraft', 'destroyDraftAll', 'storeDraftAll']),
      fetch(params) {
        this.indexCuDraft([params, this.$route.params.cu, this.$route.params.tp]);
      ...
      modalConfirmOk() {
        if (this.state == "simpan") {
					this.storeDraft(this.selectedItem.id);
				}else if (this.state == "hapus") {
          this.destroyDraft([this.selectedItem.id, this.$route.params.cu]);
        }else if (this.state == "hapus_semua") {
					this.destroyDraftAll(this.$route.params.cu);
				}else if (this.state == "simpan_semua") {
					this.storeDraftAll(this.$route.params.cu);
				}
      }
    },
    computed: {
