		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id == props.item.id }" @click="selectedRow(props.item)" class="text-nowrap">
					...
					<td v-if="!columnData[6].hide" v-html="$filters.checkStatus(props.item.terbitkan)"></td>
					<td v-if="!columnData[7].hide" v-html="$filters.checkStatus(props.item.utamakan)"></td>
					<td v-if="!columnData[8].hide" v-html="$filters.dateTime(props.item.created_at)"></td>
...
<script>
	...
	import { dateTime, checkStatus } from '../../helpers/filterHelpers';
		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.artikelStore.indexCu(params, this.$route.params.cu);
			...
			modalTutup() {
				...
				this.artikelStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.artikelStore.destroy(this.selectedItem.id);
				} else if (this.state == "updateTerbitkan"){
					this.artikelStore.updateTerbitkan(this.selectedItem.id);
				} else if (this.state == "updateUtamakan") {
					this.artikelStore.updateUtamakan(this.selectedItem.id);
				}
			}
		},
		computed:{
