								...
								<data-viewer :title="'Aset Tetap'" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" @fetch="fetchAset" :isDasar="'true'" :isNoButtonRow="'true'" v-if="isInduk == 'false' && form.aset_id == ''">
									...
									<template #item-desktop="props">
										<tr class="text-nowrap cursor-pointer" @click="selectedRow(props.item)">
											...
											<td v-html="$filters.date(props.item.tanggal)">
											</td>
											<td>
	...
	export default {
		...
		setup() {
			...
			return {
				...
				asetTetapStore,
				asetTetapJenisStore,
				asetTetapKelompokStore,
				asetTetapGolonganStore,
				asetTetapLokasiStore,
				aktivisStore,
				mitraOrangStore,
				mitraLembagaStore,
			};
		...
		methods: {
			fetch(){
				if(this.$route.meta.mode === 'edit'){
					this.asetTetapStore.edit(this.$route.params.id);	
			...
			fetchGolongan(){
				...
				this.asetTetapGolonganStore.get();
			},
			fetchLokasi(){
			...
			fetchAktivis(){
				this.aktivisStore.resetDataS();
				this.aktivisStore.get(0);
			},
			fetchAset(params){
				this.asetTetapStore.index(params);
			...
			changeKelompok(id){
				this.asetTetapJenisStore.resetDataS();
				this.asetTetapJenisStore.get(id);
			},
			changeJenis(id){
				this.asetTetapStore.generate(id);
			},
			selectedRow(item){
		...
		computed: {
			...
			form() {
				return this.asetTetapStore.data;
			},
			formStat() {
				return this.asetTetapStore.dataStat;
			},
			rules() {
				return this.asetTetapStore.rules;
			},
			options() {
				return this.asetTetapStore.options;
			},
			kode() {
				return this.asetTetapStore.kode;
			},
			kodeStat() {
			...
			itemData() {
				return this.asetTetapStore.dataS;
			},
			itemDataStat() {
				return this.asetTetapStore.dataStatS;
			},
			modelJenis() {
			...
			modelMitraOrang() {
				return this.mitraOrangStore.dataS;
			},
			modelMitraOrangStat() {
				return this.mitraOrangStore.dataStatS;
			},
			modelMitraLembaga() {
				return this.mitraLembagaStore.dataS;
			},
			modelMitraLembagaStat() {
				return this.mitraLembagaStore.dataStatS;
			},
		}
	}
</script>