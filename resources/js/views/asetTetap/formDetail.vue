...
<script>
	...
	export default {
		...
		methods: {
			...
			fetch(params){
				this.asetTetapStore.resetDataS2();
				this.asetTetapStore.indexSub([params, this.selectedItem.id]);
			},
			tutup(){
				this.$emit('tutup');
			}
		...
		computed: {
			...
			itemData() {
				return this.asetTetapStore.dataS2;
			},
			itemDataStat() {
				return this.asetTetapStore.dataStatS2;
			},
		}
	}
</script>