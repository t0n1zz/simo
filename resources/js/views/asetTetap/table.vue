		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[12].hide" v-html="$filters.date(props.item.tanggal)">
					</td>
					...
					<td v-if="!columnData[19].hide" v-html="$filters.dateTime(props.item.created_at)"></td>
					...
					<td v-if="!columnData[21].hide" v-html="$filters.dateTime(props.item.hapus_dari_laporan)"></td>
				</tr>
			</template>
...
<script>
	...
	export default {
		...
		setup() {
			...
			return { authStore, asetTetapStore };
		},
		created(){
		...
		methods: {
			fetch(params){
				if(this.tipe == 'index'){
					...
					this.asetTetapStore.indexHapus(params);
					this.excelDownloadUrl = this.kelas + '/indexHapus';
				}else if(this.tipe == 'indexSelesai'){
					this.asetTetapStore.indexSelesai(params);
			...
			modalTutup() {
				...
				this.asetTetapStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.asetTetapStore.destroy(this.selectedItem.id);
				}
				else if (this.state == 'hapusDariLaporan') {
					this.asetTetapStore.hapusDariLaporan(this.selectedItem.id);
				}
			}
		},
		computed: {
