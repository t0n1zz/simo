		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[4].hide">{{props.item.has_aset_tetap_kelompok_count}}</td>
...
<script>
	...
	export default {
		...
		setup() {
			...
			return { authStore, asetTetapGolonganStore };
		},
		data() {
			return {
				selectedItem: [],
				...
				columnData: [
					...
					},
					{
						...
						name: 'has_aset_tetap_kelompok_count',
						tipe: 'numeric',
						sort: true,
						hide: false,
						disable: false
      ...
      updateStat(value) {
		...
		methods: {
			fetch(params){
				this.asetTetapGolonganStore.index(params);
          ...
          this.modalTitle = 'Tambah golongan aset tetap';
				} else if (state == 'ubah'){
					this.modalState = 'normal1';
          this.modalColor = 'bg-primary';
          this.modalTitle = 'Ubah golongan aset tetap dengan nama: ' + this.selectedItem.name + ' ?';
				}
			},
			modalTutup() {
				this.modalShow = false;
				this.asetTetapGolonganStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.asetTetapGolonganStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
