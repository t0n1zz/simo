	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #button-desktop>
				...
				<!-- tambah -->
				<button @click.prevent="modalOpen('tambah')" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['create_aset_tetap_lokasi']">
					<i class="icon-plus3"></i> Tambah
				</button>

				<!-- ubah-->
				<button @click.prevent="modalOpen('ubah')" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['update_aset_tetap_lokasi']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalOpen('hapus')" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['destroy_aset_tetap_lokasi']" :disabled="!selectedItem.id">
			...
			<template #button-mobile>
				...
				<!-- tambah -->
				<button @click.prevent="modalOpen('tambah')" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['create_aset_tetap_lokasi']">
					<i class="icon-plus3"></i> Tambah
				</button>

				<!-- ubah-->
				<button @click.prevent="modalOpen('ubah')" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['update_aset_tetap_lokasi']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalOpen('hapus')" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['destroy_aset_tetap_lokasi']" :disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>
...
<script>
	...
	export default {
      ...
      updateStat(value) {
		...
		methods: {
			fetch(params){
				this.asetTetapLokasiStore.index(params);
          ...
          this.modalTitle = 'Ubah lokasi aset tetap dengan nama: ' + this.selectedItem.name + ' ?';
			...
			modalTutup() {
				...
				this.asetTetapLokasiStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.asetTetapLokasiStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
