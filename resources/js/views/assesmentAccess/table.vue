		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[24].hide" v-html="$filters.dateTime(props.item.created_at)"  class="text-nowrap"></td>
		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.assesmentAccessStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.assesmentAccessStore.indexCu([params,this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.assesmentAccessStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.assesmentAccessStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
			...
			...mapState(useAssesmentAccessStore, {
				itemData: 'dataS',
