		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[8].hide" v-html="$filters.dateTime(props.item.created_at)"  class="text-nowrap"></td>
		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.assesmentCulegStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.assesmentCulegStore.indexCu([params,this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.assesmentCulegStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.assesmentCulegStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
			...
			itemData() {
				return this.assesmentCulegStore.dataS;
			},
			itemDataStat() {
