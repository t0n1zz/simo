	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
          ...
          <td v-if="!columnData[32].hide">
					...
					<td v-if="!columnData[39].hide" v-html="$filters.date(props.item.ultah)"></td>
					<td v-if="!columnData[40].hide" v-html="$filters.date(props.item.bergabung)"></td>
					...
					<td v-if="!columnData[46].hide" v-html="$filters.dateTime(props.item.created_at)"  class="text-nowrap"></td>
...
export default {
  ...
  methods: {
    ...mapActions(useCuStore, ['index', 'destroy', 'resetUpdateStat']),
    fetch(params) {
      this.index(params);
