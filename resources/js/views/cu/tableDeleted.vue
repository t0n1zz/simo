	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #button-desktop>
				...
				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('restore')" class="btn btn-light btn-icon mb-1" v-if="currentUser.can && currentUser.can['update_' + kelas]" :disabled="!selectedItem.id">
					<i class="icon-checkbox-checked2"></i> Aktifkan Kembali
				</button>
			...
			<template #button-mobile>
				...
				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('restore')" class="btn btn-light btn-icon btn-block pb-1" v-if="currentUser.can && currentUser.can['update_' + kelas]" :disabled="!selectedItem.id">
					<i class="icon-checkbox-checked2"></i> Aktifkan Kembali
				</button>
          ...
          <td v-if="!columnData[11].hide">
					...
					<td v-if="!columnData[18].hide" v-html="$filters.date(props.item.ultah)"></td>
					<td v-if="!columnData[19].hide" v-html="$filters.date(props.item.bergabung)"></td>
					...
					<td v-if="!columnData[25].hide" v-html="$filters.dateTime(props.item.created_at)"  class="text-nowrap"></td>
					...
					<td v-if="!columnData[27].hide">
						<span v-html="$filters.dateTime(props.item.deleted_at)"></span>
					</td>
				</tr>
			</template>
...
export default {
  ...
  methods: {
    ...mapActions(useCuStore, ['indexDeleted', 'restore', 'resetUpdateStat']),
    fetch(params) {
      this.indexDeleted(params);
    ...
    modalConfirmOpen(state, isMobile, itemMobile) {
      ...
      if (state == "restore") {
        this.modalTitle =
          "Aktifkan kembali " + this.title + " " + this.selectedItem.name + " ini?";
    ...
    modalConfirmOk() {
      if (this.state == "restore") {
        this.restore(this.selectedItem.id);
      }
    }
  },
  computed: {
