<template>
  <div class="row">
      ...
      <count-widget 
				:count="`Terdapat ${safeCountJalan} Kegiatan Berjalan`" 
      ...
      <count-widget 
				:count="`Terdapat ${safeCountDiikuti} Kegiatan Diikuti`" 
  ...
  import countWidget from '../../components/countWidget.vue';
	...
	export default{
		...
		methods:{
			...
			fetchKegiatan(){
				const kegiatanStore = useKegiatanBKCUStore();
				kegiatanStore.fetchWidgetCountJalan();
				kegiatanStore.fetchWidgetCountDiikuti();
			},
			momentYear(){
				return moment().year();
			}	
	...
	computed: {
		safeCountJalan() {
			const count = this.countJalan;
			return (typeof count === 'number' && !isNaN(count)) ? count : 0;
		},
		safeCountDiikuti() {
			const count = this.countDiikuti;
			return (typeof count === 'number' && !isNaN(count)) ? count : 0;
		},
		currentUser() {
		...
		countJalan() {
			const kegiatanStore = useKegiatanBKCUStore();
			return kegiatanStore.widgetCountJalan;
		},
		countJalanStat() {
			const kegiatanStore = useKegiatanBKCUStore();
			return kegiatanStore.widgetCountJalanStat;
		},
		countDiikuti() {
			const kegiatanStore = useKegiatanBKCUStore();
			return kegiatanStore.widgetCountDiikuti;
		},
		countDiikutiStat() {
			const kegiatanStore = useKegiatanBKCUStore();
			return kegiatanStore.widgetCountDiikutiStat;
		},
	}
}
</script>