					...
					<table class="table table-striped  table-hover">
						...
						<tbody v-if="itemBaruStat === 'success'">
							<tr v-for="(item, index) in itemBaru" :key="index" :class="{ 'bg-info': selectedItemBaru.id === item.id }" class="text-nowrap  cursor-pointer" @click="detail(item.id)">
								<td>
									<span v-html="statusDiklat(item.status)" @click.prevent="modalKeteranganBatalOpen(item.keteranganBatal)" v-if="item.status == '6'" class="cursor-pointer"></span>
									<span v-html="statusDiklat(item.status)" v-else></span>
								</td>
								<td>
					...
					<table class="table table-striped  table-hover">
						...
						<tbody v-if="itemMulaiStat === 'success'">
							<tr v-for="(item, index) in itemMulai" :key="index" :class="{ 'bg-info': selectedItemMulai.id === item.id }" class="text-nowrap  cursor-pointer" @click="detail(item.id)">
								<td>
									<span v-html="statusDiklat(item.status)" @click.prevent="modalKeteranganBatalOpen(item.keteranganBatal)" v-if="item.status == '6'" style="cursor:pointer;"></span>
									<span v-html="statusDiklat(item.status)" v-else></span>
								</td>
								<td>
									<check-value :value="item.kode_diklat"></check-value>
...
<script>
	...
	import {  statusDiklat, date } from '../../helpers/filterHelpers';
