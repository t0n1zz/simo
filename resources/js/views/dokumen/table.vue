      ...
      updateStat(value) {
		...
		methods: {
			fetch(params){
				if(this.$route.meta.mode != 'gerakanPublik'){
					if(this.$route.params.cu == 'semua'){
						...
						this.dokumenStore.index(params);
						this.excelDownloadUrl = this.kelas;
					}else{
						this.disableColumnCu(true);
						this.dokumenStore.indexCu([params, this.$route.params.cu]);
						this.excelDownloadUrl = this.kelas + '/indexCu/' + this.$route.params.cu;
					}
				}else{
					if(this.$route.params.cu == 'semua'){
						this.disableColumnCu(false);
						this.dokumenStore.indexGerakanPublik(params);
						this.excelDownloadUrl = this.kelas;
					}else{
						this.disableColumnCu(true);
						this.dokumenStore.indexGerakanPublikCu([params, this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.dokumenStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.dokumenStore.destroy(this.selectedItem.id);
				}
			},
