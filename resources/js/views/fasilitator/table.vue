	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #button-desktop>
				...
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create'}" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['create_fasilitator']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['update_fasilitator']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['destroy_fasilitator']" :disabled="!selectedItem.id">
			...
			<template #button-mobile>
				...
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create'}" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['create_fasilitator']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['update_fasilitator']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['destroy_fasilitator']" :disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[4].hide">
						<span v-if="props.item.jenis_diklat">
              <label
                v-for="(jenis_diklat, index) in props.item.jenis_diklat"
                :key="index"
              ...
              >
                {{ jenis_diklat.name }}
					...
					<td v-if="!columnData[6].hide" v-html="$filters.dateTime(props.item.created_at)"></td>
...
<script>
	...
	import { useFasilitatorStore } from '../../stores/fasilitator';
	...
	export default {
		...
		data() {
			return {
				...
				columnData: [
					...
					{
						title: 'Jenis Diklat',
						name: 'jenisDiklat.jenisDiklat.name',
						tipe: 'string',
						sort: false,
						hide: false,
						disable: false,
      ...
      updateStat(value) {
		...
		methods: {
			...mapActions(useFasilitatorStore, ['index', 'indexCu', 'destroy', 'resetUpdateStat']),
			fetch(params){
