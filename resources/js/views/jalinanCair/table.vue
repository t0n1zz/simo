      ...
      <template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[1].hide">
						<check-value :value="props.item.jalinan_klaim.tiket_code" v-if="props.item.jalinan_klaim"></check-value>
						<span v-else>-</span>
					</td>
					<td v-if="!columnData[2].hide">
						<check-value :value="props.item.jalinan_klaim.nasabah_id" v-if="props.item.jalinan_klaim"></check-value>
						<span v-else>-</span>
					</td>
					<td v-if="!columnData[3].hide">
						<check-value :value="props.item.jalinan_klaim.anggota_cu.name" v-if="props.item.jalinan_klaim && props.item.jalinan_klaim.anggota_cu"></check-value>
						<span v-else>-</span>
					</td>
					<td v-if="!columnData[4].hide">
						<check-value :value="props.item.jalinan_klaim.anggota_cu_cu.name" v-if="props.item.jalinan_klaim && props.item.jalinan_klaim.anggota_cu_cu"></check-value>
						<span v-else>-</span>
					</td>
					<td v-if="!columnData[5].hide">
						<check-value :value="props.item.cair_tgl" valueType="date"></check-value>
					</td>
					<td v-if="!columnData[6].hide">
						<check-value :value="props.item.jumlah" valueType="currency"></check-value>
					</td>
					<td v-if="!columnData[7].hide">
						<check-value :value="props.item.no_surat"></check-value>
					</td>
					<td v-if="!columnData[8].hide">
						<span v-if="props.item.status == '0'"><span class="badge badge-warning">Menunggu</span></span>
						<span v-if="props.item.status == '1'"><span class="badge badge-success">Selesai</span></span>
					</td>
					<td v-if="!columnData[9].hide" v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
...
<script>
  ...
  export default {
    ...
    methods: {
      fetch(){
        this.jalinanKlaimStore.indexCair(this.$route.params.awal);
      },
      selectedRow(item) {
      ...
      modalConfirmOk() {
        if (this.state == "cairAll") {
          this.jalinanKlaimStore.updateCairAll([this.$route.params.awal, this.$route.params.akhir]);
        }else if(this.state == "cair"){
          if(this.selectedItem.status_klaim_cair > 0 && this.selectedItem.status_klaim_setuju == 0){
            this.jalinanKlaimStore.updateCairBatal([this.selectedItem.cu_id, this.$route.params.awal, this.$route.params.akhir]);
          }else{
            this.jalinanKlaimStore.updateCair([this.selectedItem.cu_id, this.$route.params.awal, this.$route.params.akhir]);
          }
        }
      },
      bukaData(value){
