		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.jalinanIuranStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.jalinanIuranStore.indexCu([params,this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.jalinanIuranStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.jalinanIuranStore.destroy(this.selectedItem.id);
				}
			},
		},
		computed: {
			...
			itemData() {
				return this.jalinanIuranStore.dataS;
			},
			itemDataStat() {
