			...
			<data-table :columnData="columnData" :items="itemData" :itemDataStat="itemDataStat">
				<template #item-desktop="props">
					<tr :class="{ 'bg-info': selectedItem.id == props.item.id }" @click="selectedRow(props.item)" class="text-nowrap" v-if="props.item">
						...
						<td>
							<span v-if="props.item.tanggal_lahir" v-html="$filters.date(props.item.tanggal_lahir)"></span>
							<span v-else>-</span>
						</td>
						<td>
							<span v-if="props.item.tanggal_masuk" v-html="$filters.ageDiff(props.item.tanggal_masuk, props.item.tanggal_lahir)"></span>
							<span v-else>-</span>
						</td>
						<td>
							<span v-if="props.item.tanggal_lahir" v-html="$filters.age(props.item.tanggal_lahir)"></span>
							<span v-else>-</span>
						</td>
						<td>
...
<script>
	...
	export default {
		...
		methods: {
			fetch(params){
				this.jalinanIuranStore.indexAnggota([params, this.$route.params.id, this.cu, this.lokasi]);	
			},
			selectedRow(item){
		...
		computed: {
			itemData() {
				return this.jalinanIuranStore.dataS2;
			},
			itemDataStat() {
