						...
						<form @submit.prevent="save" data-vv-scope="form">
									...
									<div class="col-md-5 mb-2">
										<div class="input-group">
											...
											<select class="form-control" name="anggota_cu_cu_id" v-model="anggota_cu_cu_id" data-width="100%" :disabled="itemData.anggota_cu_cu.length == 0 || $route.meta.mode == 'edit'" @change="changeCU($event.target.value)">
												...
												<option v-for="(anggota_cu_cu,index) in itemData.anggota_cu_cu" :value="anggota_cu_cu.id" :key="index">CU {{anggota_cu_cu.cu.name}} {{anggota_cu_cu.tp ? ' | ' + anggota_cu_cu.tp.name : ''}} | No. BA: {{ anggota_cu_cu.no_ba }} | Tanggal Jadi Anggota: <span v-if="anggota_cu_cu.tanggal_masuk" v-html="$filters.date(anggota_cu_cu.tanggal_masuk)"></span></option>
											</select>
							...
							<div v-if="($route.meta.mode == 'create' || $route.meta.mode == 'createNIK') && formStat == 'success' && form.id">
								...
								<div class="alert bg-warning text-white alert-styled-left">
									<span class="font-weight-semibold">Anggota ini sudah mengajukan Bantuan Solidaritas Jalinan  <b>pada tanggal {{ $filters.date(form.created_at) }}</b> dengan <b>status {{ $filters.statusJalinan(form.status_klaim) }}</b>, mohon periksa kembali.
									</span>
								</div>
										...
										<data-table :items="dataProduk" :columnData="columnData" :itemDataStat="dataProdukStat">
											<template #item-desktop="props">
												<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)" v-if="props.item">
													...
													<td>
														<span v-if="props.item.tanggal" v-html="$filters.ageDiff(props.item.tanggal, itemData.tanggal_lahir)"></span>
														<span v-else>-</span>
													</td>
												</tr>
											</template>	
...
<script>
	...
	import cariData from "../anggotaCu/cariData.vue";
