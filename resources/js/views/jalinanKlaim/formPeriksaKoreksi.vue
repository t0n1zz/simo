    ...
    <div class="row">
          ...
          <data-table :items="itemData" :columnData="columnData" :itemDataStat="itemDataStat">
            <template #item-desktop="props">
              <tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)" v-if="props.item">
                ...
                <td>
                  <span v-if="props.item.tanggal" v-html="$filters.date(props.item.tanggal)"></span>
                  <span v-else>-</span>
                </td>
                <td>
                  <span v-if="props.item.tanggal" v-html="$filters.ageDiff(props.item.tanggal,selectedData.anggota_cu.tanggal_lahir)">
                  </span>
                  <span v-else>-</span>
                </td>
              </tr>
              ...
              <div class="col-sm-6">
                <ul class="list list-unstyled mb-0">
                  <li>
                    ...
                    <span v-html="$filters.date(itemDataJalinan.tanggal_mati)"></span>
                  </li>
                  <li>
                    <b>Tanggal buat/pengajuan:</b> <br/>
                    <span v-html="$filters.dateTime(itemDataJalinan.created_at)"></span>
                  </li>
                  <li>
                    <b>Tanggal ubah :</b> <br/>
                    <span v-html="$filters.dateTime(itemDataJalinan.updated_at)"></span>
                  </li>
                </ul>
              </div>
      ...
      <div class="col-md-12">
        ...
        <div class="card card-body" v-if="itemDataJalinan.tanggal_mati != selectedData.tanggal_mati">
          <span>
            Tanggal cacat/meninggal dari <mark v-html="$filters.date(itemDataJalinan.tanggal_mati)"></mark> menjadi <mark v-html="$filters.date(selectedData.tanggal_mati)"></mark>
          </span>
        </div>
...
<script>
  ...
  import VeeForm from '../../components/VeeForm.vue';
	...
	export default {
      ...
      this.edit(this.selectedData.id_koreksi); 
		},
		watch: {
		},
		methods: {
      ...mapActions(useJalinanKlaimStore, ['edit', 'periksaKoreksi', 'getVerifikator']),
      ...mapActions(useAnggotaCuStore, ['indexProduk']),
