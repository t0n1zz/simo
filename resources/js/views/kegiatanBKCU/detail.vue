...
import {
	useKegiatanBKCUStore
} from '../../stores/kegiatanBKCU';
...
export default {
	...
	methods: {
		...mapActions(useKegiatanBKCUStore, {
			editKegiatan: 'edit',
			checkPesertaAct: 'checkPeserta',
			checkPanitiaAct: 'checkPanitia',
			countPesertaAct: 'countPeserta',
			countPesertaHadirAct: 'countPesertaHadir'
		}),
		fetch() {
		...
		fetchCountPeserta() {
			...
			if (this.item.tipe_tempat == 'ONLINE')
				this.countPesertaHadirAct(this.item.id);
		},
		checkTipe(tipe) {
			if (tipe == 'diklat_bkcu') {
		...
		checkPeserta() {
			this.checkPesertaAct([this.item.id, this.currentUser.id_aktivis]);
		},
		checkPanitia() {
			if (this.currentUser.aktivis) {
				this.checkPanitiaAct([this.item.id, this.currentUser.id_aktivis]);
			}
		},
		back() {
