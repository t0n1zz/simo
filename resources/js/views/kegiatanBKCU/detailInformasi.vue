          ...
          <table class="table table-borderless table-xs border-top-0 my-2">
            <tbody>
              <tr>
                ...
                <td class="text-right">
                  <span v-html="$filters.statusDiklat(item.status)"></span>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td class="text-right" v-html="$filters.dateMonth(item.mulai)"></td>
              </tr>
              <tr>
                <td class="font-weight-semibold">Tgl. Selesai:</td>
                <td class="text-right" v-html="$filters.dateMonth(item.selesai)"></td>
              </tr>
...
<script>
  ...
  methods: {
    ...mapActions(useKegiatanBKCUStore, {
      getNomorSertifikatAct: 'getNomorSertifikat',
      indexPanitiaAct: 'indexPanitia',
      indexListMateriAct: 'indexListMateri',
      destroyListMateriAct: 'destroyListMateri'
    }),
    getNomorSertifikat() {
      this.modalShow = true;
      this.modalState = 'loading';
        this.getNomorSertifikatAct(this.item.id);
      },
      fetchPanitia(){
        this.indexPanitiaAct(this.item.id);
      },
      fetchListMateri() {
				this.indexListMateriAct(this.item.id);
			},
      selectedRow(item) {
        ...
        this.modalState = 'loading';
          axios.post('/api/generateSertifikat', this.selectedItemPanitia, {
            ...
            if (this.selectedItemPanitia.asal === 'dalam') {
              saveAs(response.data, 'Piagam Kegiatan ' + this.item.name + ' - ' + this.selectedItemPanitia.aktivis_name + '.pdf');
            }else if (this.selectedItemPanitia.asal === 'luar') {
              saveAs(response.data,'Piagam Kegiatan '+ this.item.name + ' - ' + this.selectedItemPanitia.mitra_orang_name + '.pdf')
            }else if (this.selectedItemPanitia.asal === 'luar lembaga') {
              saveAs(response.data,'Piagam Kegiatan '+ this.item.name + ' - ' + this.selectedItemPanitia.mitra_lembaga_name + '.pdf')
            }
      ...
      modalConfirmOk() {
				if (this.state == 'hapusListMateri') {
					this.destroyListMateriAct([this.item.tipe, this.selectedItemListMateri.id]);
        }
			},
      modalTutup() {
