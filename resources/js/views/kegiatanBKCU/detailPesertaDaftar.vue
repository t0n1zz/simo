<template>
	<div>
		<data-viewer :title="'peserta terdaftar'" :itemData="itemDataPesertaTerdaftar"
			...
			<!-- if bkcu -->
			<template #button-desktop v-if="currentUser.id_cu == 0">
			...
			<template #button-mobile v-if="currentUser.id_cu == 0">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap"
					...
					<td>{{ props.index + 1 }}</td>
					<td>
						<span v-html="$filters.statusPeserta(props.item.status)"></span>
					</td>
					...
					<td v-if="!columnDataPesertaTerdaftar[5].hide && !columnDataPesertaTerdaftar[5].disable">
						<span v-html="props.item.datang ? $filters.date(props.item.datang) : '-'"></span>
					</td>
					<td v-if="!columnDataPesertaTerdaftar[6].hide && !columnDataPesertaTerdaftar[6].disable">
						<span v-html="props.item.pulang ? $filters.date(props.item.pulang) : '-'"></span>
					</td>
					...
					<td v-if="!columnDataPesertaTerdaftar[14].hide">
						<span>
							{{ props.item.tanggal_lahir ? $filters.date(props.item.tanggal_lahir) : '-'}}
						</span>
					</td>
...
export default {
	...
	methods: {
		...mapActions(useKegiatanBKCUStore, {
			indexPesertaAct: 'indexPeserta',
			indexPesertaCuAct: 'indexPesertaCu',
			countPesertaAct: 'countPeserta',
			destroyPeserta: 'destroyPeserta'
		}),
		selectedRow(item) {
		...
		fetchPesertaTerdaftar(params) {
				if (this.currentUser.id_cu == 0) {
					this.indexPesertaAct([params, this.item.id]);
					this.excelDownloadUrl = this.kelas + '/indexPeserta/' + this.item.id;
				} else {
					this.indexPesertaCuAct([params, this.item.id, this.currentUser.id_cu]);
		...
		fetchCountPeserta() {
			this.countPesertaAct(this.item.id);
		},
		generateSertifikat() {
			if(this.selectedItem.isGetSertifikat == 1){
				...
				this.selectedItem.peran = peran;
					axios.post('/api/generateSertifikat', this.selectedItem, {
						...
						saveAs(response.data, this.selectedItem.kegiatan_name + ' ' + this.selectedItem.name_sertifikat + '.pdf')
		...
		modalConfirmOk() {
			...
			if (this.state == 'hapusPeserta') {
				this.destroyPeserta(this.selectedItem.id);
			}
		},
		modalTutup() {
