	...
	<div>
		...
		<form @submit.prevent="handleSubmit(onValid)">
								...
								<div class="col-sm-6">
									<ul class="list list-unstyled mb-0">
										...
										<li><b>Tgl. Lahir:</b> <span
												v-html="$filters.date(formPeserta.tanggal_lahir)"></span></li>
			...
			<data-viewer :title="'Aktivis'" :columnData="columnData" :itemData="itemData" :query="query"
				...
				<template #item-desktop="props">
					<tr class="text-nowrap cursor-pointer" @click="selectedRow(props.item)">
						...
						<td v-html="$filters.checkTingkatAktivis(props.item.pekerjaan_aktif.tingkat)">
						</td>
						<td>
			...
			<div class="row" v-if="formPeserta.asal != ''">
				...
				<div class="col-md-6" v-if="item.tipe_tempat !== 'ONLINE'">
					<div class="form-group">
						<h5 :class="{ 'text-danger': errors && errors.has && errors.has('formPeserta.ukuran_baju') }">
							<i class="icon-cross2" v-if="errors && errors.has && errors.has('formPeserta.ukuran_baju')"></i>
							Ukuran Baju: 
						</h5>
...
export default {
	...
	methods: {
		...mapActions(useAktivisStore, {
			indexTingkatArr: 'indexTingkatArr',
			indexCuTingkatArr: 'indexCuTingkatArr',
		}),
		...mapActions(useMitraOrangStore, {
			indexPeserta: 'indexPeserta',
		}),
		...mapActions(useKegiatanBKCUStore, {
			updatePeserta: 'updatePeserta',
			storePeserta: 'storePeserta',
		}),
		fetchAktivis(params) {
			if (this.currentUser.id_cu == 0) {
				this.indexTingkatArr([params, this.item.id, JSON.stringify(this.tingkatArray)]);
				this.disableColumnCu(false);
			} else {
				this.indexCuTingkatArr([params, this.item.id, this.currentUser.id_cu, JSON.stringify(this.tingkatArray)])
				this.disableColumnCu(true);
			}
		},
		fetchMitra(params) {
			this.indexPeserta([params, this.item.id]);
		},
		changeAsal(asal) {
			if (asal == 'dalam') {
