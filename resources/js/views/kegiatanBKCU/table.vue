      ...
      <template #item-desktop="props">
        ...
        >
          ...
          <td v-if="!columnData[1].hide">
            <span
              v-html="formatStatusDiklat(props.item.status)"
              @click.prevent="
          ...
          <td
            ...
            v-html="formatDate(props.item.mulai)"
          ></td>
          <td
            v-if="!columnData[9].hide"
            v-html="formatDate(props.item.selesai)"
          ></td>
          ...
          <td
            ...
            v-html="formatDateTime(props.item.created_at)"
          ></td>
          <td v-if="!columnData[14].hide">
            <span
              v-if="props.item.created_at !== props.item.updated_at"
              v-html="formatDateTime(props.item.updated_at)"
            ></span>
            <span v-else>-</span>
          </td>
        </tr>
                  ...
                  <ul class="list list-unstyled mb-1">
                    <li>
                      ...
                      <span class="text-muted">{{
                        formatDate(props.item.mulai)
                      }}</span>
                    </li>
                    <li>
                      Selesai:
                      <span class="text-muted">{{
                        formatDate(props.item.selesai)
                      }}</span>
                    </li>
                    <li>
                      Durasi:
...
import { statusDiklat, date, dateTime, trimString } from "../../helpers/filterHelpers";
...
export default {
  ...
  methods: {
    formatStatusDiklat(value) {
        return statusDiklat(value);
    },
    formatDate(value) {
        return date(value);
    },
    ...
    ...mapActions(useKegiatanBKCUStore, [
      'index', 'indexJalan', 'indexDiikuti', 'indexPeriode',
      'indexDibuka', 'indexDitutup', 'indexBerjalan',
      'indexTerlaksana', 'indexMenunggu', 'indexBatal',
      'updateStatus', 'destroy', 'resetUpdateStat'
    ]),
  },
  computed: {
