      ...
      <template #item-desktop="props">
        <tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)" v-if="props.item">
          ...
          <td v-if="!columnDataPesertaTerdaftar[5].hide && !columnDataPesertaTerdaftar[5].disable" >
            <span v-html="$filters.date(props.item.datang)"></span>
          </td>
          <td v-if="!columnDataPesertaTerdaftar[6].hide && !columnDataPesertaTerdaftar[6].disable">
            <span v-html="$filters.date(props.item.pulang)"></span>
          </td>
          ...
          <td v-if="!columnDataPesertaTerdaftar[17].hide" v-html="$filters.date(props.item.tanggal_lahir)">
          </td>
          ...
          <td v-if="!columnDataPesertaTerdaftar[25].hide" v-html="$filters.dateTime(props.item.created_at)">
          </td>
        </tr>
      </template>	
    </data-viewer>
  ...
  import dataViewer from '../../components/dataviewer2.vue';
	...
	export default {
      ...
      updateStat(value) {
		...
		methods: {
			...mapActions(useKegiatanBKCUStore, ['indexSemuaPeserta', 'indexSemuaPesertaMitra', 'indexSemuaPesertaCu', 'destroyPeserta']),
      fetch(params){
        if(this.$route.params.cu == 'semua'){
          this.indexSemuaPeserta([params, this.$route.params.tipe]);
          this.excelDownloadUrl = this.kelas + '/indexSemuaPeserta/' + this.$route.params.tipe;	
        }else if(this.$route.params.cu == 'mitra'){
          this.indexSemuaPesertaMitra([params, this.$route.params.tipe]);
          this.excelDownloadUrl = this.kelas + '/indexSemuaPesertaMitra/' + this.$route.params.tipe;	
        }else{
          this.indexSemuaPesertaCu([params,this.$route.params.tipe, this.$route.params.cu]);
