	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #button-desktop>
				...
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create'}" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['create_kombas']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['update_kombas']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['destroy_kombas']" :disabled="!selectedItem.id">
			...
			<template #button-mobile>
				...
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create'}" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['create_kombas']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['update_kombas']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['destroy_kombas']" :disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[7].hide" v-html="$filters.date(props.item.tanggal_berdiri)"></td>
					<td v-if="!columnData[8].hide">
						<check-value :value="props.item.keterangan"></check-value>
					</td>
					<td v-if="!columnData[9].hide" v-html="$filters.dateTime(props.item.created_at)"></td>
...
<script>
	...
	import { useKombasStore } from '../../stores/kombas';
	...
	export default {
      ...
      updateStat(value) {
		...
		methods: {
			fetch(params){
				if(this.$route.meta.mode == 'tipe'){
					this.disableColumnCu(false);
					this.kombasStore.index([params, this.$route.params.tipe]);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.kombasStore.indexCu([params,this.$route.params.cu, this.$route.params.tipe]);
			...
			modalTutup() {
				...
				this.kombasStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.kombasStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
