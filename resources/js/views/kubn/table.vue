	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #button-desktop>
				...
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create'}" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['create_kubn']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['update_kubn']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['destroy_kubn']" :disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>

				<!-- detail-->
				<button @click.prevent="detail(selectedItem.id)" class="btn btn-light mb-1" v-if="currentUser.can && currentUser.can['index_kubn']" :disabled="!selectedItem.id">
			...
			<template #button-mobile>
				...
				<!-- tambah -->
				<router-link :to="{ name: kelas + 'Create'}" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['create_kubn']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="ubahData(selectedItem.id)" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['update_kubn']" :disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['destroy_kubn']" :disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>

				<!-- detail-->
				<button @click.prevent="detail(selectedItem.id)" class="btn btn-light btn-block mb-1" v-if="currentUser.can && currentUser.can['index_kubn']" :disabled="!selectedItem.id">
					<i class="icon-stack2"></i> Detail
				</button>
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[6].hide">
						<span v-if="props.item.tipe_anggota == 'integrasi'">
							<span class="badge bg-blue-400 align-self-center ml-auto" v-tooltip="'Data anggota kubn terhubung dengan data anggota cu'">Integrasi</span>
						</span>
						<span v-else-if="props.item.tipe_anggota == 'manual'">
							<span class="badge bg-brown-400 align-self-center ml-auto" v-tooltip="'Data anggota hanya berupa jumlah saja'">Manual</span>
						</span>
						<span v-else>-</span>
					</td>
					<td v-if="!columnData[7].hide">
						<check-value :value="props.item.jmlh_anggota" valueType="currency"></check-value>
					</td>
					<td v-if="!columnData[8].hide">
						<check-value :value="props.item.jmlh_pinjaman" valueType="currency"></check-value>
					</td>
					<td v-if="!columnData[9].hide" v-html="$filters.date(props.item.tanggal_berdiri)"></td>
					...
					<td v-if="!columnData[15].hide" v-html="$filters.dateTime(props.item.created_at)"></td>
		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.kubnStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.kubnStore.indexCu([params,this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.kubnStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.kubnStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
			...
			idCu() {
				return this.authStore.currentUser.id_cu;
			},
			modelCu() {
				return this.cuStore.dataS;
			},
			modelCuStat() {
				return this.cuStore.dataStatS;
			},
			itemData() {
				return this.kubnStore.dataS;
			},
			itemDataStat() {
