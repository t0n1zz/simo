...
<script>
	...
	import { month, year, percentage2, currency } from '../../helpers/filterHelpers';
	export default {
		components:{
			formKatex,
			appModal,
      ...
      updateStat(value) {
		...
		methods: {
			...mapActions(useLaporanCuStore, ['detailPearls', 'detailPearlsTp', 'update', 'updateTp']),
			fetch(){
				if(this.$route.meta.mode == 'detail'){
					this.detailPearls(this.$route.params.id);
				}else{
					this.detailPearlsTp(this.$route.params.id);
				}
			},
			save(){
				...
				if(this.itemData.id_tp){
					...
					this.updateTp([this.modalDetail.id,this.form]);
				}else{
					this.form.id_cu = this.modalDetail.id_cu;
					this.form.no_ba = this.modalDetail.no_ba;
					this.update([this.modalDetail.id,this.form]);
				}	
			},
