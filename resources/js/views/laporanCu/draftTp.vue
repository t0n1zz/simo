						...
						<data-table :items="itemData" :columnData="columnData" :itemDataStat="itemDataStat">
							<template #item-desktop="props">
								<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)" v-if="props.item">
									...
									<td><check-value :value="props.item.no_tp"></check-value></td>
...
<script>
	...
	export default {
		...
		watch: {},
		methods: {
			...mapActions(useLaporanTpStore, ['indexTpDraft', 'resetUpdateStat', 'destroyDraft', 'destroyDraftAll', 'storeDraftAll']),
			fetch(){
				this.indexTpDraft(this.currentUser.id);
			},
			selectedRow(item) {
