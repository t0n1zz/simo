...
<script>
	...
	export default {
		...
		methods: {
			...
			...mapActions(useTpStore, ['getCu']),
			...mapActions(useLaporanCuStore, ['create', 'edit', 'editTp', 'editDraft', 'editTpDraft']),
			...mapActions(useLaporanTpStore, ['listLaporanTp']),
			checkProfileIdCU(){
				if(this.currentUser.id_cu == 0){
					...
					this.form.id_cu = this.currentUser.id_cu;
					this.getCu(this.currentUser.id_cu);
					this.isModelTp = true;
				}
			},
			fetch(){
			...
			checkMetaEditTp(){
				...
				this.listLaporanTp([this.form.tp.id_cu, this.form.periode]);
			},
			changeTab(value) {
				this.tabName = value;
			},
