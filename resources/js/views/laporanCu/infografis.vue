...
export default {
  ...
  data(){
	...
	methods: {
		...mapActions(useLaporanCuStore, ['grafikCu', 'grafikTp', 'grafikPeriode', 'grafikTpPeriode']),
		// fetching data from database
		fetch(){
			 if(this.$route.meta.mode == 'periode'){
				this.grafikPeriode([this.query,this.$route.params.periode]);

				this.axisLabelKey = 'cu_name';
				this.titleText = 'Grafik ' + this.title + ' periode ' + this.formatPeriode(this.selectData);	
			}else if(this.$route.meta.mode == 'cu'){
				if(this.$route.params.tp == 'konsolidasi'){		
					this.grafikCu([this.query,this.$route.params.cu]);
					this.query.order_column = 'periode';
					this.query.order_direction = 'asc';
				}else{
					this.grafikTp([this.query,this.$route.params.tp]);	
				}
				this.axisLabelKey = 'periode';	
			}else if(this.$route.meta.mode == 'cuPeriode'){
				this.grafikTpPeriode([this.query,this.$route.params.cu, this.$route.params.periode]);
				this.axisLabelKey = 'tp_name';	
			}else if(this.$route.meta.mode == 'detail'){
				this.grafikCu([this.query,this.detailData.id_cu]);
				this.axisLabelKey = 'periode';
				this.query.order_column = 'periode';
				this.query.order_direction = 'asc';
			}else if(this.$route.meta.mode == 'detailTp'){
				this.grafikTp([this.query,this.detailData.id_tp]);
				this.axisLabelKey = 'periode';
				this.query.order_column = 'periode';
				this.query.order_direction = 'asc';
			}else if(this.$route.name == 'dashboard'){		
					this.grafikCu([this.query,this.currentUser.id_cu]);
					this.query.order_column = 'periode';
					this.query.order_direction = 'asc';
					this.axisLabelKey = 'periode';	
			}else{
