...
import { dateMonth } from '../../helpers/filterHelpers';
...
export default {
  ...
  data(){
	...
	methods: {
		...mapActions(useLaporanCuStore, ['grafikPearlsPeriode', 'grafikPearlsCu', 'grafikPearlsTp', 'grafikPearlsTpPeriode']),
		// fetching data from database
		fetch(){
			 if(this.$route.meta.mode == 'periode'){
				this.grafikPearlsPeriode([this.query,this.$route.params.periode]);

				this.axisLabelKey = 'cu_name';
				this.titleText = 'Grafik Pearls ' + this.title + ' periode ' + this.formatPeriode(this.selectData);	
			}else if(this.$route.meta.mode == 'cu'){
				if(this.$route.params.tp == 'konsolidasi'){
					this.grafikPearlsCu([this.query,this.$route.params.cu]);
					this.axisLabelKey = 'periode';	
				}else{
					this.grafikPearlsTp([this.query,this.$route.params.tp]);
					this.axisLabelKey = 'periode';	
				}
			}else if(this.$route.meta.mode == 'cuPeriode'){
				this.grafikPearlsTpPeriode([this.query,this.$route.params.cu, this.$route.params.periode]);
				this.axisLabelKey = 'tp_name';	
			}else if(this.$route.meta.mode == 'detail'){
				this.grafikPearlsCu([this.query,this.detailData.id_cu]);
				this.axisLabelKey = 'periode';
				this.query.order_column = 'periode';
				this.query.order_direction = 'asc';
			}else if(this.$route.meta.mode == 'detailTp'){
				this.grafikPearlsTp([this.query,this.detailData.id_tp]);
				this.axisLabelKey = 'periode';
				this.query.order_column = 'periode';
				this.query.order_direction = 'asc';	
			}else if(this.$route.name == 'dashboard'){
					this.grafikPearlsCu([this.query,this.currentUser.id_cu]);
					this.axisLabelKey = 'periode';	
			}else{
				this.grafikPearlsPeriode([this.query,this.$route.params.periode]);
