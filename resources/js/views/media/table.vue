		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.mediaStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.mediaStore.indexCu(params, this.$route.params.cu);
			...
			modalTutup() {
				...
				this.mediaStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
				this.mediaStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
