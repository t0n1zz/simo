	...
	<div>
		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
          ...
          <td v-if="!columnData[8].hide">
					...
					<td v-if="!columnData[19].hide" v-html="$filters.dateTime(props.item.created_at)"  class="text-nowrap"></td>
  ...
  methods: {
    fetch(params) {
      if(this.$route.params.cu == 'semua'){
					...
					this.mitraLembagaStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.mitraLembagaStore.indexCu([params,this.$route.params.cu]);
    ...
    modalTutup() {
      ...
      this.mitraLembagaStore.resetUpdateStat();
    },
    modalConfirmOk() {
      if (this.state == "hapus") {
        this.mitraLembagaStore.destroy(this.selectedItem.id);
      }
    }
  },
  computed: {
