  ...
  <div>
    ...
    <data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat"
      ...
      <template #item-desktop="props">
          ...
          <td v-if="!columnData[20].hide" v-html="$filters.date(props.item.tanggal_lahir)">
          </td>
          ...
          <td v-if="!columnData[27].hide" v-html="$filters.dateTime(props.item.created_at)" class="text-nowrap"></td>
    ...
    methods: {
      fetch(params) {
        if(this.$route.params.cu == 'semua'){
					...
					this.mitraOrangStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.mitraOrangStore.indexCu([params,this.$route.params.cu]);
      ...
      modalTutup() {
        ...
        this.mitraOrangStore.resetUpdateStat();
      },
      modalConfirmOk() {
        if (this.state == "hapus") {
          this.mitraOrangStore.destroy(this.selectedItem.id);
        }
      }
    },
    computed: {
