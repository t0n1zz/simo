		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query"
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id == props.item.id }" @click="selectedRow(props.item)"
					...
					<td v-if="!columnData[12].hide" v-html="formatDate(props.item.tanggal)"></td>
					<td v-if="!columnData[13].hide" v-html="formatDateTime(props.item.created_at)"></td>
					<td v-if="!columnData[14].hide">
						<span v-if="props.item.created_at !== props.item.updated_at"
							v-html="formatDateTime(props.item.updated_at)"></span>
						<span v-else>-</span>
					</td>
				</tr>
			</template>
...
export default {
	...
	mounted() {
		console.log('🗓️ Table.vue mounted. $filters:', this.$filters);
	},
	watch: {
		// check route changes
		'$route'(to, from) {
	...
	methods: {
		...mapActions(useMonitoringStore, ['index', 'indexCu', 'summary', 'destroy', 'updateTerbitkan', 'updateUtamakan', 'resetUpdateStat']),
		fetch(params) {
		...
		formatDate(value) {
            if(!filters) { console.error('❌ Filters undefined in formatDate!'); return '-'; }
			return filters.date(value);
		},
		formatDateTime(value) {
             if(!filters) { console.error('❌ Filters undefined in formatDateTime!'); return '-'; }
