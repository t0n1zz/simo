				...
				<div class="content">
					...
					<div class="row">
							...
							<div class="card card-body">
								<div class="input-group">
									...
									<select class="form-control" name="id_temuan" v-model="item.id" data-width="100%" @change="changeTemuan($event.target.value)" :disabled="modelTemuanStat == 'loading'">
										...
										<slot></slot>
										<option v-for="(te, index) in modelTemuan" :value="te.id" :key="index"><span v-if="te.tanggal" v-html="$filters.date(item.tanggal)"></span> {{te.tanggal ? ' | ' : ''}} {{te.name}} </option>
									</select>
						...
						<div class="col-lg-3 col-md-4">
							...
							<!-- detail -->
							<div class="card">
								<div class="card-header bg-light" v-if="currentUser.can && currentUser.can['update_monitoring_cu'] && item.aktivis_pemeriksa  && currentUser.id_aktivis == item.aktivis_pemeriksa.id">
								...
								<table class="table table-borderless table-xs border-top-0 my-2">
									<tbody>
										...
										<tr>
											...
											<td class="text-right">
												<span v-if="item.tanggal" v-html="$filters.date(item.tanggal)"></span>
												<span v-else>-</span>
											</td>
										</tr>
										<tr>
								...
								<div class="card-body pb-0">
									...
									<pic
										:title="'PIC'" 
										:itemData="item.aktivis_pic" 
										:itemDataStat="itemStat"></pic>
									<!-- pic bkcu -->
									<pic
										:title="'Pemeriksa'" 
										:itemData="item.aktivis_pemeriksa" 
										:itemDataStat="itemStat"></pic>
								</div>
							</div>
							...
							<div class="card card-body">
								<div class="row">
									...
									<div class="col-md-7 col-lg-9 d-none d-sm-block">
										<button class="btn btn-light" @click.prevent="modalOpen('tambah','')" v-if="currentUser.can && currentUser.can['create_monitoring_cu_keputusan']"><i class="icon-plus3"></i> Tambah Keputusan</button>
									</div>
									...
									<div class="col-md-12 d-block d-sm-none">
										<button class="btn btn-light btn-block mb-1" @click.prevent="modalOpen('tambah','')" v-if="currentUser.can && currentUser.can['create_monitoring_cu_keputusan']"><i class="icon-plus3"></i> Tambah Keputusan</button>
												...
												<div class="col-sm-12">
													<div class="card">
														<div class="card-header bg-white">
															<span class="badge badge-primary" v-html="$filters.dateTime(pc.created_at)"></span>
														</div>
														<div class="card-body">
															<div class="row">
																<div class="col-md-6">
														...
														<div class="card-body">
															...
															<div class="d-none d-sm-block">
																<!-- catatan Pemeriksa -->
																<button class="btn btn-light mb-1" @click.prevent="modalOpen('catatan',pc)" v-if="currentUser.can && currentUser.can['update_monitoring_cu'] && item.aktivis_pemeriksa && currentUser.id_aktivis == item.aktivis_pemeriksa.id">
																	<span v-if="!pc.catatan"><i class="icon-plus3"></i> Tambah Catatan Pemeriksa</span>
																	<span v-else><i class="icon-pencil5"></i> Ubah Catatan Pemeriksa</span>
																</button>
																<!-- ubah  -->
																<button class="btn btn-light mb-1" @click.prevent="modalOpen('ubah',pc)" v-if="currentUser.can && currentUser.can['create_monitoring_cu_keputusan'] && item.aktivis_pic && currentUser.id_aktivis == item.aktivis_pic.id"><i class="icon-pencil5"></i> Ubah</button>
																<!-- hapus -->
																<button class="btn btn-light mb-1" @click.prevent="modalOpen('hapus',pc)" v-if="currentUser.can && currentUser.can['create_monitoring_cu_keputusan'] && item.aktivis_pic && currentUser.id_aktivis == item.aktivis_pic.id"><i class="icon-bin2"></i> Hapus</button>
															</div>
															<!-- mobile -->
															<div class="d-block d-sm-none">
																<!-- catatan Pemeriksa -->
																<button  class="btn btn-light btn-block mb-1" @click.prevent="modalOpen('catatan',pc)" v-if="currentUser.can && currentUser.can['update_monitoring_cu'] && item.aktivis_pemeriksa && currentUser.id_aktivis == item.aktivis_pemeriksa.id">
																	<span v-if="!pc.catatan"><i class="icon-plus3"></i> Tambah Catatan Pemeriksa</span>
																	<span v-else><i class="icon-pencil5"></i> Ubah Catatan Pemeriksa</span>
																</button>
																<!-- ubah -->
																<button class="btn btn-light btn-block mb-1" @click.prevent="modalOpen('ubah',pc)" v-if="currentUser.can && currentUser.can['create_monitoring_cu_keputusan'] && item.aktivis_pic && currentUser.id_aktivis == item.aktivis_pic.id"><i class="icon-pencil5"></i> Ubah</button>
																<!-- hapus -->
																<button class="btn btn-light btn-block mb-1" @click.prevent="modalOpen('hapus',pc)" v-if="currentUser.can && currentUser.can['create_monitoring_cu_keputusan'] && item.aktivis_pic && currentUser.id_aktivis == item.aktivis_pic.id"><i class="icon-bin2"></i> Hapus</button>
															</div>
														</div>
													</div>
												</div>
...
<script>
	...
	import { useMonitoringCuStore } from '../../stores/monitoringCu';
	...
	export default {
		...
		data() {
			return {
				title: 'Detail Monitoring CU',
				titleDesc: 'Informasi detail monitoring CU',
				titleIcon: 'icon-stack2',
				level: 2,
				level2Title: 'Monitoring CU',
				kelas: 'monitoringCu',
				selectedItem: {},
				cleaveOption: {
					date: {
		...
		methods: {
			...mapActions(useMonitoringCuStore, ['edit', 'get', 'updateRekom']),
			...mapActions(useMonitoringPencapaianCuStore, {
				getPencapaian: 'get',
				destroyPencapaian: 'destroy'
			}),
			fetch() {
				this.edit(this.$route.params.id);	
			},
			fetchTemuan() {
				this.get(this.item.id_cu);
			},
			fetchPencapaian() {
				this.getPencapaian(this.$route.params.id);
			},
			back() {
			...
			modalOpen(state, selectedItem) {
				...
				if (state == 'hapus') {
					...
					this.modalState = 'normal1';
					this.modalColor = 'bg-primary';
					this.modalTitle = !selectedItem.catatan ? 'Tambah Catatan Pemeriksa' : 'Ubah Catatan Pemeriksa';
					this.modalButton = 'Ok';
					this.modalSize = 'modal-lg';
			...
			modalConfirmOk() {
				...
				if (this.state == 'hapus') {
					this.destroyPencapaian(this.selectedItem.id);
				}else if(this.state == 'rekomendasi'){
					this.updateRekom(this.selectedItem.id);
				}
			},
			modalTutup() {
				this.modalShow = false;
		...
		computed: {
			...
			...mapState(useMonitoringPencapaianCuStore, {
				itemData: 'dataS',
