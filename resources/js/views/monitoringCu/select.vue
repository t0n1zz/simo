...
<script>
	...
	export default {
		...
		methods: {
			...mapActions(useMonitoringCuStore, ['getPeriode']),
			fetch(){
				this.$router.push({name: 'monitoringKonsolidasiCu', params:{tahun: this.tahun, bulan: this.bulan} });
			},

			fetchData(){
				// if(this.modelData.length == 0){
					this.getPeriode(this.tipe);
				// }
				this.bulan = this.$route.params.bulan;
				this.tahun = this.$route.params.tahun
			},
