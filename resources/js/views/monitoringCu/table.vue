		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query"
			...
			<template #button-desktop>
				...
				<router-link :to="{ name: kelas + 'Create' }" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['create_monitoring_cu']">
					<i class="icon-plus3"></i> Tambah
				</router-link>

				<!-- ubah-->
				<button @click.prevent="goTo(selectedItem.id, 'edit')" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['update_monitoring_cu']"
				...
				<button @click.prevent="goTo(selectedItem.id, 'detail')" class="btn btn-light btn-icon mb-1"
					v-if="currentUser.can && currentUser.can['update_monitoring_cu'] || currentUser.can['create_monitoring_cu_keputusan']" :disabled="!selectedItem.id">
					<i class="icon-stack2"></i> Detail
				</button>

				<!-- hapus -->
				<button @click.prevent="modalConfirmOpen('hapus')" class="btn btn-light mb-1"
					v-if="currentUser.can && currentUser.can['destroy_monitoring_cu'] && currentUser.id_cu == 0"
					:disabled="!selectedItem.id">
					<i class="icon-bin2"></i> Hapus
				</button>

				<button @click.prevent="downloadLaporan" class="btn btn-light mb-1"
					v-if="currentUser.can && $route.params.cu != 'semua'"
					:disabled="!itemData">
			...
			<template #button-mobile>
				...
				<router-link :to="{ name: kelas + 'Create' }" class="btn btn-light btn-block mb-1"
					v-if="currentUser.can && currentUser.can['create_monitoring_cu'] && currentUser.id_cu == 0">
					<i class="icon-plus3"></i> Tambah
				</router-link>
				...
				<button @click.prevent="goTo(selectedItem.id, 'edit')" class="btn btn-light btn-block mb-1"
					v-if="currentUser.can && currentUser.can['update_monitoring_cu'] && currentUser.id_cu == 0"
					:disabled="!selectedItem.id">
					<i class="icon-pencil5"></i> Ubah
				</button>
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id == props.item.id }" @click="selectedRow(props.item)"
					...
					<td v-if="!columnData[12].hide" v-html="$filters.date(props.item.tanggal)"></td>
					...
					<td v-if="!columnData[14].hide">
						<span v-if="props.item.created_at !== props.item.updated_at"
							v-html="$filters.dateTime(props.item.updated_at)"></span>
						<span v-else>-</span>
					</td>
				</tr>
			</template>
		...
		<app-modal :show="modalShow" :state="modalState" :title="modalTitle" :content="modalContent" :size="modalSize" :color="modalColor"
		 ...
		 @backgroundClick="modalBackgroundClick">
			...
			<template #modal-body1>
				<div>
					<tgl :id_cu="this.$route.params.cu" :id_tp="this.$route.params.tp" @tutup="modalTutup" @stat="setLaporanStat" @loading="setLaporanStat" :params="query"></tgl>
				</div>
			</template>
...
export default {
	...
	created() {
		...
		this.summary(this.$route.params.cu);
	},
	watch: {
		// check route changes
		'$route'(to, from) {
	...
	methods: {
		...mapActions(useMonitoringCuStore, ['index', 'indexCu', 'destroy', 'updateTerbitkan', 'updateUtamakan', 'resetUpdateStat', 'summary']),
		fetch(params) {
			if (this.$route.params.cu == 'semua') {
				if (this.tab == 'indexSemua') {
					this.disableColumnCu(false);
					this.index([params, 'semua']);
					this.excelDownloadUrl = this.kelas + '/status/semua';
				} else {
				...
				if (this.tab == 'indexSemua') {
					...
					this.indexCu([params, this.$route.params.cu, this.$route.params.tp, 'semua']);
		...
		fetchByStatusSemua(params) {
			if (this.tab == 'indexSelesai') {
				this.disableColumnCu(false);
				this.index([params, 'selesai']);
				this.excelDownloadUrl = this.kelas + '/status/selesai';
			} else if (this.tab == 'indexKeputusan') {
				this.disableColumnCu(false);
				this.index([params, 'keputusan']);
				this.excelDownloadUrl = this.kelas + '/status/keputusan';
			} else if (this.tab == 'indexTidakKeputusan') {
				this.disableColumnCu(false);
				this.index([params, 'tidak_keputusan']);
				this.excelDownloadUrl = this.kelas + '/status/tidak_keputusan';
			}
		},
		...
		fetchByStatus(params) {
			if (this.tab == 'indexSelesai') {
				this.disableColumnCu(true);
				this.indexCu([params, this.$route.params.cu, this.$route.params.tp, 'selesai']);
				this.excelDownloadUrl = this.kelas + '/indexCu/' + this.$route.params.cu + '/' + this.$route.params.tp + '/selesai';
			} else if (this.tab == 'indexKeputusan') {
				this.disableColumnCu(true);
				this.indexCu([params, this.$route.params.cu, this.$route.params.tp, 'keputusan']);
				this.excelDownloadUrl = this.kelas + '/indexCu/' + this.$route.params.cu + '/' + this.$route.params.tp + '/keputusan';
			} else if (this.tab == 'indexTidakKeputusan') {
				this.disableColumnCu(true);
				this.indexCu([params, this.$route.params.cu, this.$route.params.tp, 'tidak_keputusan']);
		...
		modalConfirmOk() {
			if (this.state == 'hapus') {
				...
				this.updateTerbitkan(this.selectedItem.id);
			} else if (this.state == "updateUtamakan") {
				this.updateUtamakan(this.selectedItem.id);
			}
		},
		downloadLaporan() {
			this.modalShow = true;
