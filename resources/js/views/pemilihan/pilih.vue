	...
	export default {
		...
		methods: {
			...mapActions(usePemilihanStore, {
				indexCalon: 'indexCalon',
				indexCalonTerpilih: 'indexCalonTerpilih',
				storePilihan: 'storePilihan',
				storeMultiPilihan: 'storeMultiPilihan',
				resetUpdateStat: 'resetUpdateStat'
			}),
			fetch(){
				this.indexCalon(this.$route.params.name);
			},
			fetchCalonTerpilih(id){
				this.indexCalonTerpilih(id);
			},
			multi(item){	
			...
			modalConfirmOk() {
				if(this.itemData.pemilihan_max == 1){
					this.storePilihan(this.formPilihan);
				}else{
					this.storeMultiPilihan(this.formMulti);
				}
			},
		},
		computed: {
