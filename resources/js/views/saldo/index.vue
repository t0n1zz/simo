				...
				<div class="content">
					...
					<div v-if="itemDataStat == 'success'">
						...
						<div class="alert bg-danger text-white alert-styled-left" v-if="itemData.status_jalinan == 'MENINGGAL'">
							<span class="font-weight-semibold">Anggota ini sudah dinyatakan <b v-html="$filters.statusJalinan(itemData.status_jalinan)"></b>, maka tidak bisa dilakukan penambahan, pengubahan dan penghapusan data produk.
							</span>
						</div>
					...
					<ul class="nav nav-tabs nav-tabs-solid nav-justified rounded bg-light" v-if="itemDataStat == 'success'">
						<li class="nav-item" v-for="cu in itemDataCu" :key="cu.id">
							<a href="#" class="nav-link rounded-left" :class="{'active': tabName == cu.cu_id}" @click.prevent="changeTab(cu)">
								...
								{{ 'No. BA: ' + cu.no_ba }} <br/> 
								Sejak: <span v-if="cu.tanggal_masuk" v-html="$filters.date(itemData.tanggal_masuk)"></span>
							</a>
						</li>
					</ul>
...
<script>
	...
	export default {
		...
		methods: {
			...mapActions(useAnggotaCuStore, ['cariDataKTP', 'cariDataBA', 'storeProduk', 'updateProduk', 'destroyProduk']),
			...mapActions(useCuStore, {getHeader: 'getHeader'}),
			cariDataKTPWrapper(value){
				this.cariDataKTP(value);
			},
			cariDataBAWrapper(id, value){
				this.cariDataBA([id, value]);
			},
			resetData(){
				this.itemDataCu = [];
				this.itemDataProduk = [];
			...
			createProduk(value){
				this.storeProduk([this.itemData.id, value]);
			},
			editProduk(value){
				this.updateProduk([this.selectedProduk.pivot.id, value]);
			},
			...
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.destroyProduk(this.selectedProduk.pivot.id);
				}
			},
			modalTutup() {
