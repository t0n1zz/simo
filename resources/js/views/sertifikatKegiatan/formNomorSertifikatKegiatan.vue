...
<script>
	...
	created() {
		if (this.$route.meta.mode === 'edit') {
					this.editFormNomor(this.$route.params.id);	
	...
	watch: {
		...
		methods: {
			...mapActions(useSertifikatKegiatanStore, {
				editFormNomor: 'editFormNomor',
				updateNomorSertifikatKegiatan: 'updateNomorSertifikatKegiatan',
				storeNomorSertifikatKegiatan: 'storeNomorSertifikatKegiatan',
			...
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.destroyNomorSertifikatKegiatan(this.selectedItem.id);
				}
			},
			gridColor(value) {
				if(value == 1){
