					...
					<div class="row">
						...
						<div class="col-sm-12">
							<div class="card card-body">
								<input :disabled="isLoading" type="button" value="Upload" @click.prevent="submit" class="btn btn-primary ">
							</div>
						</div>
										...
										<div v-for="(file, index) in dataS" :key="index" class="col-md-4">
											<div class="card">
												<div class="card-body">
													...
													<input  type="button" value="HAPUS" v-on:click.prevent="setSelectedItem(file)" @click.prevent="modalOpen('hapus')" class="btn btn-danger ">
												</div>
											</div>
										</div>
						...
						<div class="col-sm-12">
							<div class="card card-body">
								<input v-if="currentUser.id_cu!=0" :disabled="dataS.length<=0" type="button" value="Upload Ke Draft" @click.prevent="modalOpen('upload_draft')" class="btn btn-warning mb-2">

								<input v-if="currentUser.id_cu==0" type="button" value="Upload Ke Draft" @click.prevent="modalOpen('upload_draft')" class="btn btn-warning mb-2">

								<input :disabled="!Draft" type="button" value="Simpan Draft" @click.prevent="modalOpen('simpan_draft')" class="btn btn-success mb-2">
							</div>
						</div>
	...
	export default {
		...
		watch:{
			loadingCount: function() {
				...
				if(this.loadingCount.length!=0){
					if(this.loadingCount.length == this.fileCounter){
						this.files=[]
						this.loadingCount=[]
						this.anggotaCuImportEsceteStore.index([this.currentUser.id_cu, this.currentUser.id]);
						this.fileUploadStore.index(this.currentUser.id_cu, this.currentUser.id);
					}
				}
			},
			...
			uploadStat(value){
				...
				if(value == "success"){
					this.anggotaCuImportEsceteStore.index([this.currentUser.id_cu]);
					this.fileUploadStore.index(this.currentUser.id_cu, this.currentUser.id);
		...
		methods: {
			...
			modalConfirmOk() {
				...
				if (this.state == 'hapus') {
					this.fileUploadStore.destroy(this.selectedItem.id);
				}

				if (this.state == 'upload_draft') {

					this.anggotaCuImportEsceteStore.draft([this.currentUser.id_cu, this.currentUser.id]);
				}

				if (this.state=='simpan_draft') {
					this.anggotaCuImportEsceteStore.simpanDraft(this.currentUser.id_cu);
				}
		...
		computed: {
			...
			Draft() {
				return this.anggotaCuImportEsceteStore.isDraft;
			},
			dataS() {
