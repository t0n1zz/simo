										...
										<div class="card" :class="{'text-muted' : notif.read_at != null}" @click.prevent="goToPage(notif)" style="cursor:pointer;">
											<div class="card-header bg-white header-elements-inline">
												...
												<div class="header-elements">
													{{ formatRelativeHour(notif.created_at) }}
												</div>
											</div>
											<div class="card-body">
												{{notif.data.message}}
...
<script>
  import _ from 'lodash';
	import { relativeHour } from '../../helpers/filterHelpers';
      ...
      markNotifStat(value) {
		...
		methods: {
			...
			goToPage(notif){
				...
				this.notificationStore.markRead(notif.id);
			},
			changeTab(value) {
				this.tabName = value;
			},
			...
			formatRelativeHour(value){
				return relativeHour(value);
			},
		},
		computed: {
			currentUser() {
			...
			notification() {
				return this.notificationStore.dataS;
			},
			notifStat() {
				return this.notificationStore.dataStatS;
			},
			unreadNotification() {
				return this.notificationStore.unreadNotification;
			},
			markNotifStat() {
				return this.notificationStore.markNotifStat;
			}
		},
	}
</script>