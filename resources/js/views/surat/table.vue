...
<script>
	...
	export default {
      ...
      updateStat(value) {
		...
		methods: {
			fetch(params){
				this.suratStore.indexCu([params,this.$route.params.cu, this.$route.params.tipe, this.$route.params.periode]);
			...
			modalTutup() {
				...
				this.suratStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.suratStore.destroy(this.selectedItem.id);
				}
			},
