			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[8].hide">
						<span v-if="props.item.tanggal_terima" v-html="$filters.date(props.item.tanggal_terima)"></span>
						<span v-else>-</span>
					</td>
		...
		methods: {
			fetch(params){
				if(this.$route.params.periode == 'semua'){
					this.suratMasukStore.indexCu([params,this.$route.params.cu, 'semua']);
					this.excelDownloadUrl = this.kelas + '/indexCu/' + this.$route.params.cu + '/semua';
				}else{
					this.suratMasukStore.indexCu([params,this.$route.params.cu, this.$route.params.periode]);
			...
			modalTutup() {
				...
				this.suratMasukStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.suratMasukStore.destroy(this.selectedItem.id);
				}
			},
