		...
		<data-viewer :title="title" :columnData="columnData" :itemData="itemData" :query="query" :itemDataStat="itemDataStat" :excelDownloadUrl="excelDownloadUrl" :isUploadExcel="false" @fetch="fetch">
			...
			<template #item-desktop="props">
				<tr :class="{ 'bg-info': selectedItem.id === props.item.id }" class="text-nowrap" @click="selectedRow(props.item)">
					...
					<td v-if="!columnData[10].hide" v-html="formatDate(props.item.ultah)"></td>
					...
					<td v-if="!columnData[15].hide" v-html="formatDateTime(props.item.created_at)"  class="text-nowrap"></td>
					<td v-if="!columnData[16].hide">
						<span v-if="props.item.created_at !== props.item.updated_at" v-html="formatDateTime(props.item.updated_at)"></span>
						<span v-else>-</span>
					</td>
				</tr>
			</template>
...
<script>
	...
	import { date, dateTime } from '../../helpers/filterHelpers';
		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.tpStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.tpStore.indexCu([params,this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.tpStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.tpStore.destroy(this.selectedItem.id);
				}
			},
			formatDate(value) { return date(value); },
			formatDateTime(value) { return dateTime(value); },
		},
		computed: {
