...
<script>
	...
	export default {
		...
		methods: {
			fetch(){
				if(this.$route.meta.mode === 'edit'){
					this.umkmStore.edit(this.$route.params.id);	
			...
			fetchCU(){
				if(this.modelCu.length == 0){
					this.cuStore.getHeader(this.currentUser.id_pus);
				}
			},
			changeCU(id){
				this.form.id_cu = id;
		...
		computed: {
			...
			form() {
				return this.umkmStore.data;
			},
			formStat() {
				return this.umkmStore.dataStat;
			},
			rules() {
				return this.umkmStore.rules;
			},
			options() {
