		...
		methods: {
			fetch(params){
				if(this.$route.params.cu == 'semua'){
					...
					this.umkmStore.index(params);
					this.excelDownloadUrl = this.kelas;
				}else{
					this.disableColumnCu(true);
					this.umkmStore.indexCu([params,this.$route.params.cu]);
			...
			modalTutup() {
				...
				this.umkmStore.resetUpdateStat();
			},
			modalConfirmOk() {
				if (this.state == 'hapus') {
					this.umkmStore.destroy(this.selectedItem.id);
				}
			}
		},
		computed: {
