      ...
      <template #item-desktop="props">
        ...
        >
          ...
          <td v-if="!columnData[6].hide">
            <span v-if="props.item.aktivis && props.item.aktivis.pekerjaan_aktif" v-html="$filters.checkTingkatAktivis(props.item.aktivis.pekerjaan_aktif.tingkat)"></span>
            <span v-else>-</span>
					</td>
          ...
          <td v-if="!columnData[8].hide" v-html="$filters.checkStatus(props.item.status)"></td>
          <td
            v-if="!columnData[9].hide"
            class="text-nowrap"
            v-html="$filters.dateTime(props.item.login)"
          ></td>
          <td
            v-if="!columnData[10].hide"
            class="text-nowrap"
            v-html="$filters.dateTime(props.item.created_at)"
          ></td>
          <td v-if="!columnData[11].hide">
            <span
              v-if="props.item.created_at !== props.item.updated_at"
              v-html="$filters.dateTime(props.item.updated_at)"
            ></span>
            <span v-else>-</span>
          </td>
        </tr>
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <ul class="list list-unstyled mb-0">
                      ...
                      <li><b>Tingkat:</b> 
                        <span v-if="selectedItem.aktivis && selectedItem.aktivis.pekerjaan_aktif" v-html="$filters.checkTingkatAktivis(selectedItem.aktivis.pekerjaan_aktif.tingkat)"></span>
                        <span v-else>-</span>
                      </li>
                      <li><b>Jabatan:</b>
                  ...
                  <div class="col-sm-6">
                    <ul class="list list-unstyled mb-0">
                      ...
                      <li><b>Login:</b> <span v-html="$filters.dateTime(selectedItem.login)"></span></li>
                      <li><b>Tgl. Buat:</b> <span v-html="$filters.dateTime(selectedItem.created_at)"></span></li>
                      <li><b>Tgl. Ubah:</b> <span v-html="$filters.dateTime(selectedItem.updated_at)"></span></li>
                    </ul>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    modalTutup() {
      ...
      this.userStore.resetUpdateStat();
    },
    modalConfirmOk() {
      if (this.state == "hapus") {
        this.userStore.destroy(this.selectedItem.id);
      } else if (this.state == "resetPassword") {
        this.userStore.updateResetPassword(
          this.selectedItem.id
        );
      } else if (this.state == "updateStatus") {
        this.userStore.updateStatus(
          this.selectedItem.id
        );
