							...
							<div v-show="tabName == 'info'">
								...
								<div class="card">
									<div class="card-header bg-white header-elements-inline">
										<h5 class="card-title">Voting | Status: <span v-html="$filters.checkStatus(form.status)"></span></h5>
      ...
      updateStat(value) {
		...
		methods: {
			...mapActions(useVotingStore, {
				edit: 'edit',
				indexDataSuara: 'indexDataSuara',
				indexDataTanggapan: 'indexDataTanggapan',
				updateStatus: 'updateStatus',
				destroySuara: 'destroySuara',
			...
			fetchDataSuara(params){
				this.indexDataSuara([params, this.$route.params.id]);
				this.excelDownloadUrl = this.kelas + '/indexDataSuara/' + this.$route.params.id;
			},
			fetchDataTanggapan(params){
				this.indexDataTanggapan([params, this.$route.params.id]);
			...
			modalConfirmOk() {
				if (this.state == "status"){
					this.updateStatus([this.form.id, this.form.id_cu]);
				}else if (this.state == "hapus_suara"){
					this.destroySuara(this.selectedSuara.id);
				}
			},
		},
		computed: {
